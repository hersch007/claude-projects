...
| Start Performance integration (8.9.0, optional) | Two small bridge files under `sp-views/` (`fruth-quotes.php`, `fruth-pricing.php`) that the Start Performance platform's core plugin can `require` directly as sidebar-embedded views, if that platform's core plugin is active on the same site. Entirely inert otherwise -- gated behind `sqs_pricing_calculator_sp_mode()` (`function_exists('sp_register_addon')`). **Corrected 2026-09-16, see `docs/Site-Topology.md`**: this is in fact installed and active on the live `startwebservicesbackup.com/fruth/` site itself (reachable at `/fruth/sp-app/?view=fruth-quotes`) -- the "Start Performance not installed on the live Fruth site" note in earlier revisions of this doc was wrong. See `docs/CHANGELOG.md` 8.9.0 for the full design. |
...
- `https://startwebservicesbackup.com/fruth/portal/` → ✅ **Fixed** (per the earlier `/fruth/` slug issue, now resolved), but **superseded as of 2026-09-16**: this URL now forwards to `https://startwebservicesbackup.com/fruth/sp-app/`, the Start Performance dashboard. The standalone `[sqs_portal]` tile page is no longer the front door for real users on this site — see `docs/Site-Topology.md`. The plugin-side `sqs_pricing_calculator_portal_url()` logic is unchanged and still correct; the redirect is a site-level routing choice, not a code path.
