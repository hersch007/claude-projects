...
function sqs_pricing_calculator_users_table_name() {
    global $wpdb;
    return $wpdb->prefix . 'sqs_users';
}
...
function sqs_pricing_calculator_user_can_quote_ajax() {
    ...
    if ( ! empty( $_SESSION['sqs_calc_sales_logged_in'] ) && true === $_SESSION['sqs_calc_sales_logged_in'] ) {
        return true;
    }
    if ( current_user_can( 'manage_options' ) ) {
        return true;
    }
    // Start Performance mode: any logged-in team member (admin or agent) can
    // use the Quote Builder -- matches the agreed role split (Admin gets
    // Sales + Data Editor + user management; Agent gets Sales only).
    if ( sqs_pricing_calculator_sp_mode() && function_exists( 'sp_get_current_team_member' ) && sp_get_current_team_member() ) {
        return true;
    }
    return false;
}
...
function sqs_pricing_calculator_clean_user_role( $role ) {
    $role = sanitize_text_field( (string) $role );
    return in_array( $role, array( 'sales', 'admin', 'both' ), true ) ? $role : 'sales';
}

function sqs_pricing_calculator_clean_user_status( $status ) {
    $status = sanitize_text_field( (string) $status );
...
function sqs_pricing_calculator_activate() {
    ...
    // Individual user accounts (replacing the two shared Sales/Editor logins).
    // role: 'sales' | 'admin' | 'both'. status: 'active' | 'inactive' -- loose
    // varchar with in-code whitelisting, matching the existing quotes.status
    // convention (see sqs_pricing_calculator_clean_quote_status()) rather than
    // a lookup table, which would be overkill for a handful of named users.
    $users_table = sqs_pricing_calculator_users_table_name();
    $users_sql = "CREATE TABLE $users_table (
        ...
        can_manage_users tinyint(1) unsigned NOT NULL DEFAULT 0,
        created_at datetime NOT NULL,
        updated_at datetime NOT NULL,
...
/**
 * One-time seed of wp_sqs_users from the two legacy shared-credential option
 * pairs. Deliberately NOT wired to register_activation_hook alone -- this
 * plugin is deployed by overwriting the file, which does not re-fire
 * WordPress's activation hook, so this must also be called defensively on
 * every relevant page load (see the two login shortcodes) the same way
 * sqs_pricing_calculator_activate() already is. Guarded by an option flag so
 * it only actually runs once. Safe to call from a page load context (no
 * upgrade.php dependency here, dbDelta already ran in activate()).
 */
function sqs_pricing_calculator_maybe_migrate_users() {
    if ( get_option( 'sqs_calc_users_migrated' ) ) {
        return;
    }
    global $wpdb;
    $users_table = sqs_pricing_calculator_users_table_name();
    ...
    $legacy_accounts = array(
        array(
            'display_name'  => 'Sales Team (legacy)',
            'username'      => get_option( 'sqs_calc_sales_user', 'sales_team' ),
            'password_hash' => get_option( 'sqs_calc_sales_pass_hash', '' ),
            'role'          => 'sales',
        ),
        array(
            'display_name'  => 'Admin (legacy)',
            'username'      => get_option( 'sqs_calc_editor_user', 'sales_admin' ),
            'password_hash' => get_option( 'sqs_calc_editor_pass_hash', '' ),
            'role'          => 'admin',
        ),
    );
    ...
    foreach ( $legacy_accounts as $account ) {
        $username = sanitize_text_field( (string) $account['username'] );
        // Nothing to migrate for this side if it was never configured.
        if ( '' === $username || '' === (string) $account['password_hash'] ) {
            continue;
        }

        // Collision guard: nothing today prevents the Sales and Editor
        // usernames from having been set identically. Suffix the second one
        // rather than letting the UNIQUE constraint fail or silently
        // dropping a working login -- either would lock someone out.
        if ( in_array( $username, $used_usernames, true ) ) {
            $original_username = $username;
            $username .= '_' . $account['role'];
            add_action( 'admin_notices', function () use ( $username, $original_username, $account ) {
                echo '<div class="notice notice-warning"><p><strong>Quote Builder:</strong> your Sales and Data Editor logins both used the username "' . esc_html( $original_username ) . '". The ' . esc_html( $account['role'] ) . ' account was renamed to <code>' . esc_html( $username ) . '</code> so both logins keep working -- update it any time in Login &amp; Branding.</p></div>';
            } );
        }
        ...
        $wpdb->insert( $users_table, array(
            ...
            'role'          => sqs_pricing_calculator_clean_user_role( $account['role'] ),
            'status'        => 'active',
            'created_at'    => $now,
            'updated_at'    => $now,
        ) );
    ...
    update_option( 'sqs_calc_users_migrated', 1 );
}

/**
 * Looks up one active user by username only (no role filter in the SQL).
 * Callers verify the password first with wp_check_password(), then check
 * role compatibility -- this order avoids a minor timing side-channel and
 * gives a clear "wrong password" vs "right password, wrong role" distinction.
 */
function sqs_pricing_calculator_find_active_user( $username ) {
    $username = sanitize_text_field( (string) $username );
    if ( '' === $username ) {
        return null;
    }
    global $wpdb;
    $table = sqs_pricing_calculator_users_table_name();
    $row = $wpdb->get_row(
        $wpdb->prepare(
...
/**
 * Finds whichever published page contains the given shortcode and returns
 * its permalink, falling back to a relative path under home_url() if no
 * such page exists yet. Always relative -- never a hardcoded domain.
 */
function sqs_pricing_calculator_shortcode_page_url( $shortcode_tag, $fallback_path ) {
    global $wpdb;
...
function sqs_pricing_calculator_portal_url() {
    return sqs_pricing_calculator_shortcode_page_url( 'sqs_portal', '/' );
}
...
function sqs_app_bar_styles( $embedded = false ) { ?>
<style>
...
/* Restyled to match the Start Performance app: light card toolbar, SP ghost
   secondaries + accent-primary Save (inherits Fruth's brand accent via --sp-accent). */
.sqs-quote-toolbar{display:flex;align-items:center;gap:8px;flex-wrap:wrap;background:#ffffff;border:1px solid #e2e8f0;border-radius:12px;padding:11px 14px;box-sizing:border-box;width:100%;font-family:inherit;margin-bottom:18px;position:sticky;top:58px;z-index:9998;box-shadow:0 1px 3px rgba(15,23,42,.07);}
.sqs-bar-btn{display:inline-flex !important;align-items:center !important;justify-content:center !important;gap:6px !important;height:36px !important;padding:0 15px !important;border-radius:8px !important;font-size:13px !important;font-weight:600 !important;cursor:pointer !important;border:1px solid #e2e8f0 !important;text-decoration:none !important;white-space:nowrap !important;transition:background .15s,border-color .15s,color .15s !important;line-height:1 !important;font-family:inherit !important;background:#f1f5f9 !important;color:#475569 !important;box-shadow:none !important;box-sizing:border-box !important;vertical-align:middle !important;}
.sqs-bar-btn:hover{background:#e2e8f0 !important;border-color:#cbd5e1 !important;color:#1e293b !important;}
.sqs-bar-btn-primary{background:var(--sp-accent,#CC1F1F) !important;border-color:transparent !important;color:#ffffff !important;}
.sqs-bar-btn-primary:hover{background:var(--sp-accent-dark,#b01818) !important;border-color:transparent !important;color:#ffffff !important;}
.sqs-bar-sep{width:1px;height:22px;background:#e2e8f0;flex-shrink:0;margin:0 2px;}
.sqs-toolbar-spacer{flex:1 1 auto;min-width:12px;}
.sqs-status-control{display:inline-flex;align-items:center;gap:8px;background:#f8fafc;border:1px solid #e2e8f0;border-radius:8px;padding:0 8px 0 12px;height:36px;box-sizing:border-box;vertical-align:middle;white-space:nowrap;}
.sqs-status-label{font-size:11px;font-weight:800;text-transform:uppercase;letter-spacing:.08em;color:#64748b;line-height:1;flex-shrink:0;}
.sqs-bar-select{display:block;height:28px;width:112px;border:1px solid #cbd5e1;border-radius:7px;background:#fff;color:#0f172a;font-size:12px;font-weight:700;padding:0 6px 0 8px;font-family:inherit;outline:none;box-sizing:border-box;margin:0;align-self:center;appearance:auto;-webkit-appearance:menulist;}
/* Embedded in Start Performance the platform's global <select> rule (font-size:14px;padding:10px 12px) overrides the compact bar style above and vertically clips the value text. Re-assert a readable, un-clipped style with enough specificity + !important to win regardless of load order. */
.sqs-status-control select.sqs-bar-select{height:32px !important;min-height:32px !important;font-size:13px !important;line-height:normal !important;padding:4px 28px 4px 10px !important;box-sizing:border-box !important;-webkit-appearance:menulist !important;appearance:menulist !important;}
.sqs-bar-status{display:inline-flex;align-items:center;height:36px;font-size:11px;font-weight:700;color:#94a3b8;background:transparent;border:0;padding:0 4px;white-space:nowrap;line-height:1;}
...
<?php if ( $embedded ) : ?>
/* Embedded contexts (e.g. Start Performance) never render .sqs-app-bar, so
   there's no 58px bar above this toolbar to stick below -- pin it to the
   top of its scroll container instead of leaving that gap. (A negative
   margin-top was tried here to also cancel out that scroll container's own
   32px padding, but sticky positioning + a negative margin reaching into a
   scrolling ancestor's padding is an unreliable CSS combination across
   browsers -- reverted. See sqs_pricing_calculator_render()'s own <style>
   block for the .sqs-calc padding fix instead, which is safe because it
   only removes padding this plugin owns, not a parent's.) */
.sqs-quote-toolbar{top:0;}
<?php endif; ?>
</style><?php }

function sqs_pricing_calculator_reset_page() {
    if ( ! current_user_can( 'manage_options' ) ) return;
    // Defensive schema check -- this plugin is deployed by overwriting the
    // file, which never re-fires register_activation_hook, so a new column
    // (e.g. can_manage_users, added in 8.8.0) will not exist yet on this
    // page's very first load after an upgrade unless dbDelta runs here too.
    // Without this, the $wpdb->replace() below fails silently against a
    // missing column and this "it just works no matter what" recovery page
    // stops being that.
    ...
    if ( isset($_POST['sqs_do_reset']) && check_admin_referer('sqs_reset_credentials') ) {
        ...
        // Break-glass recovery: unconditionally (re)create one guaranteed
        // admin/both-role row in wp_sqs_users. Must not depend on reading
        // anything else from that table first -- this has to work even if
        // every other row in it is broken or misconfigured.
        $wpdb->replace( sqs_pricing_calculator_users_table_name(), array(
            'display_name'     => 'Emergency Admin',
            'username'         => 'sales_admin_reset',
            'password_hash'    => wp_hash_password( 'ChangeMe123!' ),
            'role'             => 'both',
            'status'           => 'active',
            'can_manage_users' => 1,
            'created_at'       => $now,
        ...
        // Also reset the legacy shared-credential options -- these remain
        // the fallback path in the login code if wp_sqs_users is ever
        // unreadable (e.g. restrictive DB permissions), so this button
        // needs to fix that path too, not just the new table.
        ...
        $msg = 'Done! Emergency account (works for the Quote Builder, Data Editor, and User Management): sales_admin_reset / ChangeMe123!  |  Legacy fallback logins were also reset -- Sales: sales_team / ChangeMe123!  |  Editor: sales_admin / ChangeMe123!';
    }

    global $wpdb;
    $users_table = sqs_pricing_calculator_users_table_name();
    $active_admin_count = (int) $wpdb->get_var( "SELECT COUNT(*) FROM $users_table WHERE status = 'active' AND role IN ('admin','both')" );
    $active_user_admin_count = (int) $wpdb->get_var( "SELECT COUNT(*) FROM $users_table WHERE status = 'active' AND can_manage_users = 1" );
    ...
    <div style="background:#fff;border:1px solid #dcdcde;border-radius:12px;padding:20px;max-width:500px;margin:16px 0;">
        ...
        <p>Total user accounts: <strong><?php echo esc_html( $total_users ); ?></strong></p>
        <p>Active admin-level accounts: <?php echo $active_admin_count > 0 ? '<strong style="color:green;">' . esc_html( $active_admin_count ) . '</strong>' : '<strong style="color:red;">0 -- nobody can reach the Data Editor right now</strong>'; ?></p>
        <p>Active "Can manage users" accounts: <?php echo $active_user_admin_count > 0 ? '<strong style="color:green;">' . esc_html( $active_user_admin_count ) . '</strong>' : '<strong style="color:red;">0 -- nobody can reach User Management (wp-admin or front-end) right now</strong>'; ?></p>
    </div>
    <div style="background:#fff;border:2px solid #c62828;border-radius:12px;padding:24px;max-width:560px;">
        <h2 style="margin-top:0;color:#c62828;">Emergency Reset</h2>
        <p>Use this only if you're locked out. It creates (or resets) one guaranteed-working account, <code>sales_admin_reset</code>, password <code>ChangeMe123!</code>, with access to the Quote Builder, the Data Editor, <strong>and</strong> User Management (so you can immediately fix things from the front-end Portal without a second WordPress-admin trip) -- then also resets the legacy fallback logins to the same password, in case the accounts table itself is unreachable.</p>
        <p><strong>Change this password immediately after logging back in</strong>, via the User Accounts section on the Login &amp; Branding page or the front-end User Management page.</p>
...
function sqs_pricing_calculator_portal_shortcode( $atts ) {
    ...
    $sales_url = $atts['sales_url'] ?: sqs_pricing_calculator_sales_quote_builder_url();
    $admin_url = $atts['admin_url'] ?: sqs_pricing_calculator_data_editor_url();
    ob_start(); ?>
...
.sqs-portal-bottom{background:linear-gradient(160deg,#e8ecf2 0%,#f0f3f7 40%,#e2e8ef 100%);padding:28px 44px 40px;border-top:1px solid #d0d7e3}.sqs-portal-logo{display:block;max-width:220px;height:auto;margin:0 auto 20px}.sqs-portal-kicker{font-size:10px;font-weight:700;color:#8896a8;text-transform:uppercase;letter-spacing:.18em;margin-bottom:8px;font-family:'DM Mono',monospace}.sqs-portal-title{font-size:26px;font-weight:800;color:#0f1623;margin:0 0 6px;letter-spacing:-.02em}.sqs-portal-sub{font-size:14px;color:#667085;margin:0 0 32px}.sqs-portal-btn{display:flex;align-items:center;gap:14px;width:100%;padding:16px 20px;border-radius:14px;text-decoration:none;margin-bottom:12px;transition:transform .15s,box-shadow .15s;border:2px solid transparent;box-sizing:border-box}.sqs-portal-btn:hover{transform:translateY(-2px)}.sqs-portal-btn:last-of-type{margin-bottom:0}.sqs-portal-btn-icon{width:44px;height:44px;border-radius:10px;display:flex;align-items:center;justify-content:center;font-size:20px;flex-shrink:0}.sqs-portal-btn-label{font-size:15px;font-weight:800;display:block;line-height:1.2}.sqs-portal-btn-desc{font-size:12px;display:block;margin-top:2px;opacity:.75}.sqs-portal-btn.sales{background:linear-gradient(135deg,#fef2f2,#fee2e2);border-color:#fecaca;color:#7f1d1d;box-shadow:0 4px 16px rgba(198,40,40,.12)}.sqs-portal-btn.sales:hover{box-shadow:0 8px 24px rgba(198,40,40,.20);border-color:#c62828}.sqs-portal-btn.sales .sqs-portal-btn-icon{background:#c62828;color:white}.sqs-portal-btn.admin{background:linear-gradient(135deg,#eff6ff,#dbeafe);border-color:#bfdbfe;color:#1e3a5f;box-shadow:0 4px 16px rgba(2,132,199,.10)}.sqs-portal-btn.admin:hover{box-shadow:0 8px 24px rgba(2,132,199,.18);border-color:#0284c7}.sqs-portal-btn.admin .sqs-portal-btn-icon{background:#0284c7;color:white}.sqs-portal-btn.useradmin{background:linear-gradient(135deg,#f5f3ff,#ede9fe);border-color:#ddd6fe;color:#4c1d95;box-shadow:0 4px 16px rgba(109,40,217,.10)}.sqs-portal-btn.useradmin:hover{box-shadow:0 8px 24px rgba(109,40,217,.18);border-color:#6d28d9}.sqs-portal-btn.useradmin .sqs-portal-btn-icon{background:#6d28d9;color:white}.sqs-portal-footer{margin-top:28px;font-size:11px;color:#9ca3af;font-family:'DM Mono',monospace}
    ...
    <div class="sqs-portal-wrap"><div class="sqs-portal-card">
        ...
        <a href="<?php echo esc_url( sqs_pricing_calculator_user_admin_url() );?>" class="sqs-portal-btn useradmin">
            <div class="sqs-portal-btn-icon">&#128100;</div>
            <div><span class="sqs-portal-btn-label">User Management</span><span class="sqs-portal-btn-desc">Add, edit &amp; deactivate accounts</span></div>
        </a>
        <?php do_action( 'sqs_portal_extra_tiles' ); ?>
...
function sqs_pricing_calculator_sales_quote_builder_url() {
    // Fallback path is relative to home_url(), which already resolves to
    // this site's /fruth/ base -- do not repeat "/fruth" here or the
    // fallback (only ever used if the Quotes page doesn't exist yet) would
    // double up to /fruth/fruth/quotes/.
    return sqs_pricing_calculator_shortcode_page_url( 'sqs_pricing_calculator', '/quotes/' );
}

function sqs_pricing_calculator_data_editor_url() {
    // When Start Performance is active on this site, the standalone
    // [sqs_pricing_data_admin] page may no longer exist/be published --
    // route straight to the SP-embedded view instead of falling through to
    // a hardcoded /editor/ path that isn't a real page anymore.
    if ( sqs_pricing_calculator_sp_mode() ) {
        return home_url( '/sp-app/?view=fruth-pricing' );
    }
    return sqs_pricing_calculator_shortcode_page_url( 'sqs_pricing_data_admin', '/editor/' );
}

function sqs_pricing_calculator_user_admin_url() {
    return sqs_pricing_calculator_shortcode_page_url( 'sqs_user_admin', '/users/' );
}
...
// ── AJAX: User Management (Login & Branding admin page + front-end
//    [sqs_user_admin] page) ───────────────────────────────────────────────
// Reachable either by a real WordPress admin (manage_options) or by a
// wp_sqs_users account with can_manage_users=1 authenticated through the
// front-end session login -- so nopriv registration is required, same as
// the Sales/Data-Editor/Customer-Database AJAX endpoints. Uses its own
// nonce action (sqs_users_admin) rather than the shared sqs_quote_actions
// nonce; the nonce is not the authorization boundary here (it never is in
// this codebase) -- sqs_pricing_calculator_user_can_manage_users_ajax()
// below is, and it re-checks the account's status/capability fresh from
// the DB on every call rather than trusting anything cached in the session.
add_action( 'wp_ajax_sqs_list_users',        'sqs_ajax_list_users' );
add_action( 'wp_ajax_nopriv_sqs_list_users', 'sqs_ajax_list_users' );
function sqs_ajax_list_users() {
    if ( ! sqs_pricing_calculator_user_can_manage_users_ajax() ) {
        wp_send_json_error( array( 'message' => 'Not authorized.' ), 403 );
    }
    check_ajax_referer( 'sqs_users_admin', 'nonce' );

    global $wpdb;
    $table = sqs_pricing_calculator_users_table_name();
    $rows = $wpdb->get_results( "SELECT id, display_name, username, role, status, can_manage_users, created_at, last_login_at FROM $table ORDER BY display_name ASC", ARRAY_A );
    wp_send_json_success( array( 'users' => $rows ? $rows : array() ) );
}

add_action( 'wp_ajax_sqs_save_user',        'sqs_ajax_save_user' );
add_action( 'wp_ajax_nopriv_sqs_save_user', 'sqs_ajax_save_user' );
function sqs_ajax_save_user() {
    if ( ! sqs_pricing_calculator_user_can_manage_users_ajax() ) {
        wp_send_json_error( array( 'message' => 'Not authorized.' ), 403 );
    }
    check_ajax_referer( 'sqs_users_admin', 'nonce' );
    ...
    $role             = sqs_pricing_calculator_clean_user_role( isset( $_POST['role'] ) ? wp_unslash( $_POST['role'] ) : '' );
    $status           = sqs_pricing_calculator_clean_user_status( isset( $_POST['status'] ) ? wp_unslash( $_POST['status'] ) : '' );
    $can_manage_users = ! empty( $_POST['can_manage_users'] ) && '0' !== $_POST['can_manage_users'] ? 1 : 0;

    if ( '' === $username ) {
        wp_send_json_error( array( 'message' => 'Username is required.' ), 400 );
    }
    ...
    if ( $existing && (int) $existing['id'] !== $user_id ) {
        wp_send_json_error( array( 'message' => 'That username is already in use.' ), 409 );
    }

    if ( $user_id ) {
        // Two independent lockout guards -- neither replaces the other,
        // since role (Sales/Admin/Both) and can_manage_users are separate
        // capabilities that can each independently be the last one standing.
        // No "can't edit your own account" restriction beyond these guards --
        // matches the rest of this system (no role special-cases self-edit
        // either); a manage-users account editing itself is expected usage.
        $would_lose_admin = ( 'sales' === $role ) || ( 'inactive' === $status );
        if ( $would_lose_admin && sqs_pricing_calculator_is_last_active_admin( $user_id ) ) {
            wp_send_json_error( array( 'message' => 'This is the last active admin account -- add or promote another admin before changing this one.' ), 409 );
        }
        $would_lose_user_admin = ( 0 === $can_manage_users ) || ( 'inactive' === $status );
        if ( $would_lose_user_admin && sqs_pricing_calculator_is_last_active_user_admin( $user_id ) ) {
            wp_send_json_error( array( 'message' => 'This is the last account that can manage users -- add or promote another one before changing this one.' ), 409 );
        }
        ...
        $wpdb->update( $table, $fields, array( 'id' => $user_id ) );
        $message = 'User updated.';
    } else {
        if ( '' === $password ) {
            wp_send_json_error( array( 'message' => 'A password is required for new accounts.' ), 400 );
        }
    ...
    wp_send_json_success( array( 'user_id' => $user_id, 'message' => $message ) );
}

add_action( 'wp_ajax_sqs_deactivate_user',        'sqs_ajax_deactivate_user' );
add_action( 'wp_ajax_nopriv_sqs_deactivate_user', 'sqs_ajax_deactivate_user' );
function sqs_ajax_deactivate_user() {
    if ( ! sqs_pricing_calculator_user_can_manage_users_ajax() ) {
        wp_send_json_error( array( 'message' => 'Not authorized.' ), 403 );
    }
    check_ajax_referer( 'sqs_users_admin', 'nonce' );
    ...
    // Two independent lockout guards -- see the matching comment in
    // sqs_ajax_save_user() above.
    if ( sqs_pricing_calculator_is_last_active_admin( $user_id ) ) {
        wp_send_json_error( array( 'message' => 'This is the last active admin account and cannot be deactivated -- add or promote another admin first.' ), 409 );
    }
    if ( sqs_pricing_calculator_is_last_active_user_admin( $user_id ) ) {
        wp_send_json_error( array( 'message' => 'This is the last account that can manage users and cannot be deactivated -- add or promote another one first.' ), 409 );
    }

    global $wpdb;
    $wpdb->update(
        sqs_pricing_calculator_users_table_name(),
...
/**
 * Authorization gate for the User Management AJAX endpoints. Always
 * re-checks the account fresh from the DB rather than trusting anything
 * cached in the session, so a mid-session deactivation/demotion takes
 * effect immediately rather than only at next login.
 */
function sqs_pricing_calculator_user_can_manage_users_ajax() {
    if ( current_user_can( 'manage_options' ) ) {
        return true;
    }
    sqs_pricing_calculator_maybe_start_session();
    if ( empty( $_SESSION['sqs_calc_useradmin_logged_in'] ) ) {
        return false;
    }
    $user_id = isset( $_SESSION['sqs_calc_useradmin_user_id'] ) ? absint( $_SESSION['sqs_calc_useradmin_user_id'] ) : 0;
    if ( ! $user_id ) {
        return false;
    }
    global $wpdb;
    $table = sqs_pricing_calculator_users_table_name();
    $row = $wpdb->get_row( $wpdb->prepare( "SELECT status, can_manage_users FROM $table WHERE id = %d", $user_id ), ARRAY_A );
    return $row && 'active' === $row['status'] && 1 === (int) $row['can_manage_users'];
}

/**
 * True only if $user_id is currently the sole active admin-level account
 * (role 'admin' or 'both'). Used to block edits/deactivation that would
 * lock the whole team out of the Data Editor / User Management screen.
 */
function sqs_pricing_calculator_is_last_active_admin( $user_id ) {
    global $wpdb;
    $table = sqs_pricing_calculator_users_table_name();
    $row = $wpdb->get_row( $wpdb->prepare( "SELECT id, role, status FROM $table WHERE id = %d", $user_id ), ARRAY_A );
    if ( ! $row || 'active' !== $row['status'] || ! in_array( $row['role'], array( 'admin', 'both' ), true ) ) {
        return false; // Not currently an active admin -- nothing to protect.
    }
    $other_admins = (int) $wpdb->get_var(
        $wpdb->prepare(
            "SELECT COUNT(*) FROM $table WHERE id != %d AND status = 'active' AND role IN ('admin','both')",
            $user_id
        )
    );
    return 0 === $other_admins;
}

/**
 * True only if $user_id is currently the sole active account with
 * can_manage_users=1. Independent of sqs_pricing_calculator_is_last_active_admin()
 * -- role and "can manage users" are separate capabilities, so both guards
 * must run on every save/deactivate, neither replaces the other. Without
 * this, the front-end User Management page could brick itself with no
 * recovery short of a WordPress-admin trip to Reset Logins.
 */
function sqs_pricing_calculator_is_last_active_user_admin( $user_id ) {
    global $wpdb;
    $table = sqs_pricing_calculator_users_table_name();
    $row = $wpdb->get_row( $wpdb->prepare( "SELECT id, status, can_manage_users FROM $table WHERE id = %d", $user_id ), ARRAY_A );
    if ( ! $row || 'active' !== $row['status'] || 1 !== (int) $row['can_manage_users'] ) {
        return false; // Doesn't currently manage users -- nothing to protect.
    }
    $other_user_admins = (int) $wpdb->get_var(
        $wpdb->prepare(
            "SELECT COUNT(*) FROM $table WHERE id != %d AND status = 'active' AND can_manage_users = 1",
            $user_id
        )
    );
    return 0 === $other_user_admins;
}

function sqs_pricing_calculator_access_page() {
    if ( ! current_user_can( 'manage_options' ) ) return;
    // Defensive schema check -- see the matching comment in
    // sqs_pricing_calculator_reset_page().
    sqs_pricing_calculator_activate();
    $msg = '';
    ...
    sqs_pricing_calculator_maybe_migrate_users();
    $users_nonce = wp_create_nonce( 'sqs_users_admin' );
    ?>
    <div class="wrap">
    ...
    <?php sqs_pricing_calculator_render_user_accounts_panel( $users_nonce ); ?>
    </div>
    <?php
}

/**
 * Shared User Accounts list/add-edit/deactivate panel -- rendered both from
 * the WordPress admin "Login & Branding" page and from the front-end
 * [sqs_user_admin] shortcode, so accounts with the "Can manage users"
 * capability never have to touch wp-admin. $nonce must match whatever
 * check_ajax_referer() the caller's AJAX handlers expect ('sqs_users_admin').
 */
function sqs_pricing_calculator_render_user_accounts_panel( $nonce ) {
    ?>
    <div style="background:#fff;border:1px solid #dcdcde;border-radius:12px;padding:24px;margin:24px 0;max-width:900px;">
        <h2 style="margin-top:0;border-bottom:1px solid #eee;padding-bottom:10px;">&#128100; User Accounts</h2>
        <p class="description">Individual logins for the Quote Builder (<code>[sqs_pricing_calculator]</code>) and Data Editor (<code>[sqs_pricing_data_admin]</code>). Role controls which one an account can sign into: <strong>Sales</strong> (Quote Builder only), <strong>Admin</strong> (Data Editor only), or <strong>Both</strong>. "Can manage users" is independent of Role -- any account, regardless of Role, can also be allowed to manage other accounts here.</p>
        <div id="sqs-users-msg"></div>
        <table class="widefat striped" style="margin-bottom:16px;">
            <thead><tr><th>Name</th><th>Username</th><th>Role</th><th>Manages Users</th><th>Status</th><th>Last Login</th><th></th></tr></thead>
            <tbody id="sqs-users-rows"><tr><td colspan="7">Loading&hellip;</td></tr></tbody>
        </table>
        <button type="button" class="button button-primary" id="sqs-user-add-btn">+ Add User</button>

        <div id="sqs-user-form-wrap" style="display:none;margin-top:20px;padding-top:20px;border-top:1px solid #eee;max-width:480px;">
            <h3 id="sqs-user-form-title" style="margin-top:0;">Add User</h3>
            <input type="hidden" id="sqs-user-id" value="">
            <p><label><strong>Display Name</strong><br><input type="text" id="sqs-user-display-name" class="regular-text" style="margin-top:4px;"></label></p>
            <p><label><strong>Username</strong><br><input type="text" id="sqs-user-username" class="regular-text" autocomplete="off" style="margin-top:4px;"></label></p>
            <p><label><strong>Password</strong><br><input type="password" id="sqs-user-password" class="regular-text" autocomplete="new-password" style="margin-top:4px;"></label><br><span class="description" id="sqs-user-password-hint">Required for new accounts.</span></p>
            <p><label><strong>Role</strong><br>
                <select id="sqs-user-role" style="margin-top:4px;">
                    <option value="sales">Sales (Quote Builder)</option>
                    <option value="admin">Admin (Data Editor)</option>
                    <option value="both">Both</option>
                </select>
            </label></p>
            <p><label><input type="checkbox" id="sqs-user-can-manage-users" style="margin-right:6px;vertical-align:middle;"> <strong>Can manage users</strong></label><br><span class="description">Lets this account create, edit, and deactivate other accounts (from either this screen or the front-end User Management page) -- independent of the Role above.</span></p>
            <p><label><strong>Status</strong><br>
                <select id="sqs-user-status" style="margin-top:4px;">
                    <option value="active">Active</option>
                    <option value="inactive">Inactive</option>
                </select>
            </label></p>
            <p>
                <button type="button" class="button button-primary" id="sqs-user-save-btn">Save User</button>
                <button type="button" class="button" id="sqs-user-cancel-btn">Cancel</button>
            </p>
        </div>
    </div>
    <script>
    (function(){
        ...
        var rowsEl = document.getElementById('sqs-users-rows');
        var msgEl = document.getElementById('sqs-users-msg');
        var formWrap = document.getElementById('sqs-user-form-wrap');
        var formTitle = document.getElementById('sqs-user-form-title');
        var idField = document.getElementById('sqs-user-id');
        var nameField = document.getElementById('sqs-user-display-name');
        var userField = document.getElementById('sqs-user-username');
        var passField = document.getElementById('sqs-user-password');
        var passHint = document.getElementById('sqs-user-password-hint');
        var roleField = document.getElementById('sqs-user-role');
        var manageUsersField = document.getElementById('sqs-user-can-manage-users');
        var statusField = document.getElementById('sqs-user-status');
        var usersCache = [];
        ...
        function roleLabel(r) {
            return r === 'admin' ? 'Admin' : (r === 'both' ? 'Both' : 'Sales');
        }

        function showMsg(text, isError) {
            msgEl.innerHTML = '<div class="notice ' + (isError ? 'notice-error' : 'notice-success') + '" style="margin:0 0 12px;"><p>' + esc(text) + '</p></div>';
        }
        ...
        function renderRows() {
            if (!usersCache.length) {
                rowsEl.innerHTML = '<tr><td colspan="7">No users yet -- add one below.</td></tr>';
                return;
            }
            rowsEl.innerHTML = usersCache.map(function(u){
                return '<tr>' +
                    '<td>' + esc(u.display_name) + '</td>' +
                    '<td>' + esc(u.username) + '</td>' +
                    '<td>' + esc(roleLabel(u.role)) + '</td>' +
                    '<td>' + (Number(u.can_manage_users) === 1 ? 'Yes' : '—') + '</td>' +
                    '<td>' + (u.status === 'active' ? 'Active' : '<span style="color:#b32d2e;">Inactive</span>') + '</td>' +
                    '<td>' + esc(u.last_login_at || 'Never') + '</td>' +
                    '<td><button type="button" class="button button-small sqs-user-edit-btn" data-id="' + u.id + '">Edit</button> ' +
                        (u.status === 'active' ? '<button type="button" class="button button-small sqs-user-deactivate-btn" data-id="' + u.id + '">Deactivate</button>' : '') +
                    '</td>' +
                '</tr>';
            }).join('');

            rowsEl.querySelectorAll('.sqs-user-edit-btn').forEach(function(btn){
                btn.addEventListener('click', function(){ openForm(parseInt(btn.getAttribute('data-id'), 10)); });
            });
            rowsEl.querySelectorAll('.sqs-user-deactivate-btn').forEach(function(btn){
                btn.addEventListener('click', function(){ deactivate(parseInt(btn.getAttribute('data-id'), 10)); });
            });
        }

        function loadUsers() {
            post('sqs_list_users').then(function(res){
                if (res.success) {
                    usersCache = res.data.users || [];
                    renderRows();
                } else {
                    rowsEl.innerHTML = '<tr><td colspan="7">Could not load users.</td></tr>';
                }
            });
        }
        ...
        function openForm(userId) {
            var u = null;
            if (userId) {
                usersCache.forEach(function(row){ if (parseInt(row.id, 10) === userId) u = row; });
            }
            idField.value = u ? u.id : '';
            nameField.value = u ? u.display_name : '';
            userField.value = u ? u.username : '';
            passField.value = '';
            passHint.textContent = u ? 'Leave blank to keep current password.' : 'Required for new accounts.';
            roleField.value = u ? u.role : 'sales';
            manageUsersField.checked = u ? Number(u.can_manage_users) === 1 : false;
            statusField.value = u ? u.status : 'active';
            formTitle.textContent = u ? 'Edit User' : 'Add User';
        ...
        function saveUser() {
            var payload = {
                user_id: idField.value || '',
                display_name: nameField.value,
                username: userField.value,
                password: passField.value,
                role: roleField.value,
                can_manage_users: manageUsersField.checked ? '1' : '0',
                status: statusField.value
            };
            if (!payload.username) { showMsg('Username is required.', true); return; }
            post('sqs_save_user', payload).then(function(res){
                if (res.success) {
                    showMsg((res.data && res.data.message) || 'Saved.', false);
                    closeForm();
                    loadUsers();
                } else {
                    showMsg((res.data && res.data.message) || 'Save failed.', true);
                }
            });
        }
        ...
        function deactivate(userId) {
            ...
            post('sqs_deactivate_user', { user_id: userId }).then(function(res){
                if (res.success) {
                    showMsg((res.data && res.data.message) || 'Deactivated.', false);
                    loadUsers();
                } else {
                    showMsg((res.data && res.data.message) || 'Could not deactivate.', true);
                }
            });
        }

        document.getElementById('sqs-user-add-btn').addEventListener('click', function(){ openForm(0); });
        document.getElementById('sqs-user-save-btn').addEventListener('click', saveUser);
        document.getElementById('sqs-user-cancel-btn').addEventListener('click', closeForm);
...
function sqs_pricing_calculator_admin_page() {
    ...
    <div class="wrap sqs-admin-wrap">
        ...
        (function(){
            ...
            function input(v, cls, field){ return `<input type="text" class="${cls||'sqs-cell'}"${field ? ` data-field="${esc(field)}"` : ''} value="${esc(v)}">`; }
            ...
            // productCodes is the one table that's a list of {code, desc} objects rather
            // than a flat map/list/matrix -- without this, the generic list renderer below
            // would treat each object as a single scalar cell and flatten/corrupt it on save.
            const objectListFields = { productCodes: { fields: ['code','desc'], labels: ['Code','Description'] } };
            function renderObjectList(key, value){
                const def = objectListFields[key];
                let rows = (value || []).map(obj => {
                    obj = obj || {};
                    const cells = def.fields.map(f => `<td>${input(obj[f], 'sqs-obj-value', f)}</td>`).join('');
                    return `<tr>${cells}<td><button type="button" class="button-link-delete sqs-delete-row">Remove</button></td></tr>`;
                }).join('');
                const head = def.labels.map(l => `<th>${esc(l)}</th>`).join('') + `<th></th>`;
                return `<div class="sqs-editor-table-wrap"><table class="sqs-editor-table" data-kind="objectlist" data-fields='${esc(JSON.stringify(def.fields))}'><thead><tr>${head}</tr></thead><tbody>${rows}</tbody></table></div>`;
            }
            ...
            function renderEditor(el){
                ...
                else if (objectListFields[key]) html = renderObjectList(key, value);
            ...
            function serializeCard(card){
                ...
                } else if (objectListFields[key]) {
                    out = Array.from(editor.querySelectorAll('tbody tr')).map(tr => {
                        const obj = {};
                        tr.querySelectorAll('.sqs-obj-value').forEach(inp => { obj[inp.dataset.field] = inp.value.trim(); });
                        return obj;
            ...
            document.querySelectorAll('.sqs-add-row').forEach(btn => btn.addEventListener('click', function(){
                ...
                if (kind === 'objectlist') {
                    ...
                    const cells = fields.map(f => `<td>${input('','sqs-obj-value',f)}</td>`).join('');
                    table.querySelector('tbody').insertAdjacentHTML('beforeend', `<tr>${cells}<td><button type="button" class="button-link-delete sqs-delete-row">Remove</button></td></tr>`);
                }
            }));
...
function sqs_pricing_calculator_frontend_is_logged_in() {
    if ( ! empty( $_SESSION['sqs_calc_frontend_logged_in'] ) && true === $_SESSION['sqs_calc_frontend_logged_in'] ) {
        return true;
    }
    // Start Performance mode: only admin-role team members get Data Editor
    // access, matching the agreed role split.
    if ( sqs_pricing_calculator_sp_mode() && function_exists( 'sp_is_admin_member' ) && sp_is_admin_member() ) {
        return true;
    }
    return false;
}
...
/**
 * Shared "save all data tables" handler -- called from the standalone
 * Data Editor shortcode's POST handling and from the Start Performance
 * fruth-pricing view's own POST handling, so the save logic exists in one
 * place. Reads $_POST['sqs_data'] directly (not passed as a parameter),
 * matching how it was always invoked before this was extracted.
 */
function sqs_pricing_calculator_frontend_handle_save_all() {
...
function sqs_pricing_calculator_frontend_editor_shortcode() {
    ...
    if ( isset( $_POST['sqs_frontend_action'] ) ) {
        ...
        if ( 'login' === $action ) {
            ...
            $user_row   = sqs_pricing_calculator_find_active_user( $posted_user );
            $login_ok   = false;
            $wrong_role = false;

            if ( $user_row && wp_check_password( $posted_pass, $user_row['password_hash'] ) ) {
                if ( in_array( $user_row['role'], array( 'admin', 'both' ), true ) ) {
                    $login_ok = true;
                } else {
                    $wrong_role = true;
                }
            } elseif ( ! $user_row ) {
                // Legacy fallback -- safety net in case dbDelta/migration hasn't
                // run yet on this host. Remove once migration is confirmed
                // healthy in production for a full release cycle.
            ...
            if ( $login_ok ) {
                ...
                if ( $user_row ) {
                    $_SESSION['sqs_calc_current_user_id']   = (int) $user_row['id'];
                    $_SESSION['sqs_calc_current_user_name'] = $user_row['display_name'];
                    global $wpdb;
                    $wpdb->update(
                        sqs_pricing_calculator_users_table_name(),
                        array( 'last_login_at' => current_time( 'mysql' ) ),
                        array( 'id' => $user_row['id'] )
                    );
                }
                sqs_pricing_calculator_frontend_clear_message();
            } else {
                $msg = $wrong_role ? 'This account does not have Data Editor access.' : 'Login failed. Please check the username and password.';
                sqs_pricing_calculator_frontend_set_message( $msg );
            }
        } elseif ( 'logout' === $action ) {
            unset( $_SESSION['sqs_calc_frontend_logged_in'], $_SESSION['sqs_calc_current_user_id'], $_SESSION['sqs_calc_current_user_name'] );
...
function sqs_pricing_calculator_render_frontend_editor( $message = '', $embedded = false ) {
    global $wpdb;
    $table = sqs_pricing_calculator_table_name();
    $rows = $wpdb->get_results( "SELECT data_key, data_label, data_json, updated_at FROM $table ORDER BY id ASC", ARRAY_A );
    $rows = sqs_pricing_calculator_filter_hidden_data_rows( $rows );
    ...
    <div class="sqs-data-app">
        ...
        (function(){
            ...
            // productCodes is a list of {code, desc} objects, not a flat map/list/matrix --
            // without this the generic list renderer below would flatten/corrupt it on save.
            const objectListFields = {productCodes:{fields:['code','desc'],labels:['Code','Description']}};
            function renderObjectList(key,value){const def=objectListFields[key];let rows=(value||[]).map(obj=>{obj=obj||{};const cells=def.fields.map(f=>`<td>${input(obj[f],'sqs-obj-value',f)}</td>`).join('');return `<tr>${cells}<td><button type="button" class="sqs-data-remove sqs-delete-row">Remove</button></td></tr>`;}).join('');const head=def.labels.map(l=>`<th>${esc(l)}</th>`).join('')+`<th></th>`;return `<div class="sqs-data-table-wrap"><table class="sqs-data-table" data-kind="objectlist" data-fields='${esc(JSON.stringify(def.fields))}'><thead><tr>${head}</tr></thead><tbody>${rows}</tbody></table></div>`;}
            ...
            function renderEditor(el){const key=el.dataset.key;const value=JSON.parse(el.dataset.value||'null');let html='';if(key==='defaults')html=renderDefaults(value);else if(objectListFields[key])html=renderObjectList(key,value);else if(matrixHeaders[key])html=renderMatrix(key,value);else if(Array.isArray(value))html=renderList(key,value);else if(value&&typeof value==='object')html=renderMap(key,value);else html='<p>Use Advanced View.</p>';el.innerHTML=html;}
...
function sqs_pricing_calculator_shortcode() {
    sqs_pricing_calculator_maybe_start_session();
    // Defensive schema check -- see the matching comment in
    // sqs_pricing_calculator_reset_page(). Pre-existing gap (this shortcode
    // never called activate() even before 8.8.0); fixed alongside the same
    // bug found on the other pages that touch wp_sqs_users.
    sqs_pricing_calculator_activate();
    sqs_pricing_calculator_maybe_migrate_users();
    if ( isset( $_POST['sqs_sales_action'] ) ) {
        if ( 'logout' === $_POST['sqs_sales_action'] ) {
            unset( $_SESSION['sqs_calc_sales_logged_in'], $_SESSION['sqs_calc_current_user_id'], $_SESSION['sqs_calc_current_user_name'] );
            wp_safe_redirect( sqs_pricing_calculator_logout_url() ); exit;
        }
        if ( 'login' === $_POST['sqs_sales_action'] ) {
            ...
            $user_row  = sqs_pricing_calculator_find_active_user( $u );
            $login_ok  = false;
            $wrong_role = false;

            if ( $user_row && wp_check_password( $p, $user_row['password_hash'] ) ) {
                if ( in_array( $user_row['role'], array( 'sales', 'both' ), true ) ) {
                    $login_ok = true;
                } else {
                    $wrong_role = true;
                }
            ...
            if ( $login_ok ) {
                ...
                $msg = $wrong_role ? 'This account does not have Sales Portal access.' : 'Login failed.';
                ob_start(); sqs_pricing_calculator_render_sales_login( $msg ); return ob_get_clean();
            }
        }
    }
...
// Lets a wp_sqs_users account with can_manage_users=1 manage other accounts
// entirely from the public site, without ever needing a WordPress login.
// Session flags here are deliberately namespaced (sqs_calc_useradmin_*)
// rather than reusing the shared sqs_calc_current_user_id/name keys, so
// signing into this page in one tab can never overwrite which person the
// Quote Builder's "Prepared By" auto-fill thinks is active in another tab
// of the same browser session.

add_shortcode( 'sqs_user_admin', 'sqs_pricing_calculator_user_admin_shortcode' );
function sqs_pricing_calculator_user_admin_shortcode() {
    sqs_pricing_calculator_maybe_start_session();
    // Defensive schema check -- see the matching comment in
    // sqs_pricing_calculator_reset_page().
    sqs_pricing_calculator_activate();
    ...
    if ( isset( $_POST['sqs_useradmin_action'] ) ) {
        if ( 'logout' === $_POST['sqs_useradmin_action'] ) {
            unset( $_SESSION['sqs_calc_useradmin_logged_in'], $_SESSION['sqs_calc_useradmin_user_id'], $_SESSION['sqs_calc_useradmin_user_name'] );
            wp_safe_redirect( sqs_pricing_calculator_logout_url() ); exit;
        }
        if ( 'login' === $_POST['sqs_useradmin_action'] ) {
            $u = isset( $_POST['sqs_useradmin_user'] ) ? sanitize_text_field( wp_unslash( $_POST['sqs_useradmin_user'] ) ) : '';
            $p = isset( $_POST['sqs_useradmin_pass'] ) ? (string) wp_unslash( $_POST['sqs_useradmin_pass'] ) : '';

            $user_row = sqs_pricing_calculator_find_active_user( $u );
            $login_ok = $user_row
                && wp_check_password( $p, $user_row['password_hash'] )
                && 1 === (int) $user_row['can_manage_users'];
            // No legacy-option fallback here -- "can manage users" is a new
            // capability that didn't exist before wp_sqs_users, so there is
            // no shared-credential equivalent to fall back to.

            if ( $login_ok ) {
                session_regenerate_id( true );
                $_SESSION['sqs_calc_useradmin_logged_in']  = true;
                $_SESSION['sqs_calc_useradmin_user_id']    = (int) $user_row['id'];
                $_SESSION['sqs_calc_useradmin_user_name']  = $user_row['display_name'];
                global $wpdb;
                $wpdb->update(
                    sqs_pricing_calculator_users_table_name(),
                    array( 'last_login_at' => current_time( 'mysql' ) ),
                ...
                ob_start(); sqs_pricing_calculator_render_user_admin_login( 'Login failed, or this account cannot manage users.' ); return ob_get_clean();
            }
        }
    }

    if ( empty( $_SESSION['sqs_calc_useradmin_logged_in'] ) ) {
        ob_start(); sqs_pricing_calculator_render_user_admin_login(); return ob_get_clean();
    }
    ob_start(); sqs_pricing_calculator_render_user_admin_panel(); return ob_get_clean();
}

function sqs_pricing_calculator_render_user_admin_login( $msg = '' ) {
    $co = get_option('sqs_company_name','Quote Builder');
    $lg = get_option('sqs_logo_url','');
    ?>
    <link href="https://fonts.googleapis.com/css2?family=DM+Sans:wght@400;700;800&display=swap" rel="stylesheet"/>
    <style>.sqs-lw{min-height:100vh;display:flex;align-items:center;justify-content:center;background:#edf0f4;font-family:'DM Sans',sans-serif}.sqs-lc{background:#fff;border-radius:20px;padding:48px 40px;width:100%;max-width:400px;box-shadow:0 8px 32px rgba(10,15,30,.12);border:1px solid #e2e5eb}.sqs-lc img{display:block;max-width:200px;height:auto;margin:0 auto 16px}.sqs-lk{font-size:10px;font-weight:700;color:#6d28d9;text-transform:uppercase;letter-spacing:.14em;text-align:center;margin-bottom:8px}.sqs-lt{font-size:22px;font-weight:800;color:#0f1623;text-align:center;margin:0 0 4px}.sqs-ls{font-size:13px;color:#667085;text-align:center;margin:0 0 24px}.sqs-lf{margin-bottom:14px}.sqs-lf label{display:block;font-size:11px;font-weight:700;color:#374151;text-transform:uppercase;letter-spacing:.06em;margin-bottom:5px}.sqs-lf input{width:100%;border:1px solid #d0d5dd;border-radius:10px;padding:11px 14px;font-size:14px;color:#0f1623;box-sizing:border-box}.sqs-lb{width:100%;background:#6d28d9;color:#fff;border:none;border-radius:10px;padding:13px;font-size:14px;font-weight:700;cursor:pointer;margin-top:6px}.sqs-lb:hover{background:#5b21b6;}.sqs-le{background:#fef2f2;border:1px solid #fecaca;border-radius:8px;padding:10px;font-size:13px;color:#c62828;font-weight:600;margin-bottom:14px;text-align:center}</style>
    <div class="sqs-lw"><div class="sqs-lc">
        <?php if($lg):?><img src="<?php echo esc_url($lg);?>" alt="<?php echo esc_attr($co);?>"/><?php endif;?>
        <div class="sqs-lk">&#128100; User Management</div>
        <h2 class="sqs-lt">User Management</h2>
        <p class="sqs-ls">Sign in to manage accounts</p>
        <?php if($msg):?><div class="sqs-le"><?php echo esc_html($msg);?></div><?php endif;?>
        <form method="post">
            <input type="hidden" name="sqs_useradmin_action" value="login"/>
            <div class="sqs-lf"><label>Username</label><input type="text" name="sqs_useradmin_user" autocomplete="username" required/></div>
            <div class="sqs-lf"><label>Password</label><input type="password" name="sqs_useradmin_pass" autocomplete="current-password" required/></div>
            <button class="sqs-lb" type="submit">Sign In &rarr;</button>
        </form>
        <div style="text-align:center;margin-top:16px;">
            <a href="<?php echo esc_url( sqs_pricing_calculator_portal_url() ); ?>" style="display:inline-flex;align-items:center;justify-content:center;gap:6px;padding:13px 20px;background:#0f1623;color:#ffffff;border:2px solid #0f1623;border-radius:12px;font-size:14px;font-weight:700;text-decoration:none;width:100%;box-sizing:border-box;letter-spacing:.02em;">&#8592; Return to Home</a>
...
function sqs_pricing_calculator_render_user_admin_panel() {
    $name = isset( $_SESSION['sqs_calc_useradmin_user_name'] ) ? sanitize_text_field( $_SESSION['sqs_calc_useradmin_user_name'] ) : '';
    $nonce = wp_create_nonce( 'sqs_users_admin' );
    sqs_pricing_calculator_frontend_styles();
    ?>
    <style>
    /* Scoped to .sqs-user-panel only -- the shared User Accounts panel
       markup uses WordPress-admin CSS classes (.widefat, .button, .notice)
       that render correctly inside wp-admin for free but have no styling
       at all on the public front end. This gives them a matching look here
       without touching the wp-admin page's own (already-correct) styling. */
    .sqs-user-panel h2{font-family:'DM Sans',Arial,sans-serif;font-size:20px;}
    .sqs-user-panel .description{color:#64748b;font-size:13px;line-height:1.5;}
    .sqs-user-panel table.widefat{width:100%;border-collapse:collapse;background:#fff;border:1px solid #e2e8f0;border-radius:10px;overflow:hidden;font-size:13px;}
    .sqs-user-panel table.widefat th{background:#f8fafc;text-align:left;font-size:11px;text-transform:uppercase;letter-spacing:.04em;color:#64748b;padding:10px 12px;border-bottom:1px solid #e2e8f0;}
    .sqs-user-panel table.widefat td{padding:10px 12px;border-bottom:1px solid #edf2f7;vertical-align:middle;}
    .sqs-user-panel table.widefat tr:last-child td{border-bottom:0;}
    .sqs-user-panel .button{display:inline-flex;align-items:center;justify-content:center;border:1px solid #cbd5e1;background:#fff;color:#334155;border-radius:8px;padding:7px 14px;font-size:12px;font-weight:700;cursor:pointer;text-decoration:none;font-family:inherit;}
    .sqs-user-panel .button:hover{background:#f1f5f9;border-color:#94a3b8;}
    .sqs-user-panel .button-primary{background:#6d28d9;border-color:#6d28d9;color:#fff;}
    .sqs-user-panel .button-primary:hover{background:#5b21b6;border-color:#5b21b6;}
    .sqs-user-panel .button-small{padding:5px 10px;font-size:11px;}
    .sqs-user-panel .button-large{padding:11px 18px;font-size:14px;}
    .sqs-user-panel input[type=text],.sqs-user-panel input[type=password]{border:1px solid #cbd5e1;border-radius:8px;padding:9px 11px;font-size:13px;width:100%;box-sizing:border-box;font-family:inherit;}
    .sqs-user-panel select{border:1px solid #cbd5e1;border-radius:8px;padding:9px 11px;font-size:13px;font-family:inherit;background:#fff;}
    .sqs-user-panel label strong{font-size:12px;color:#334155;}
    .sqs-user-panel .notice{border-radius:8px;padding:10px 12px;font-size:13px;border:1px solid;}
    .sqs-user-panel .notice-success{background:#ecfdf5;border-color:#bbf7d0;color:#14532d;}
    .sqs-user-panel .notice-error{background:#fef2f2;border-color:#fecaca;color:#991b1b;}
    .sqs-user-panel .signed-in-as{margin:0 0 14px;font-size:12px;color:#64748b;}
    </style>
    <div class="sqs-data-app">
        <div class="sqs-app-bar">
            <div class="sqs-app-bar-left">
                ...
                <div class="sqs-app-bar-divider"></div>
                <?php endif; ?>
                <span class="sqs-app-bar-name">User Management</span>
            </div>
            <div class="sqs-app-bar-right">
                <a href="<?php echo esc_url( sqs_pricing_calculator_portal_url() ); ?>" class="sqs-bar-btn sqs-bar-btn-home">&#8592; Home</a>
                <form method="post" style="margin:0;display:inline-block;">
                    <input type="hidden" name="sqs_useradmin_action" value="logout"/>
                    <button type="submit" class="sqs-bar-btn sqs-bar-btn-signout">Sign Out</button>
                </form>
            </div>
        </div>
        <div style="height:12px;background:#f5f7fb;"></div>
        <div class="sqs-data-grid">
            <div class="sqs-user-panel">
                <p class="signed-in-as">Signed in as <strong><?php echo esc_html( $name ); ?></strong></p>
                <?php sqs_pricing_calculator_render_user_accounts_panel( $nonce ); ?>
            </div>
        </div>
    </div>
    <?php
...
function sqs_pricing_calculator_render( $embedded = false ) {
    $sqs_data = sqs_pricing_calculator_get_data();
    // Display convenience only, not an audit trail -- preparedBy is free
    // text inside the saved quote's JSON, not a foreign key to this user.
    $sqs_data['currentUserName'] = sqs_pricing_calculator_current_user_display_name();
    $sqs_data_json = wp_json_encode( $sqs_data );
    $sqs_quote_ajax_url = admin_url( 'admin-ajax.php' );
    $sqs_quote_nonce = wp_create_nonce( 'sqs_quote_actions' );
    if ( $embedded ) {
        // sqs_app_bar_styles() (the CSS behind .sqs-quote-toolbar, .sqs-bar-btn,
        // .sqs-status-control, .sqs-load-modal -- everything this view's kept
        // toolbar/Load-Quote-modal need) is normally printed via the wp_head
        // hook. Embedding contexts like Start Performance's app shell build
        // their own <head> and never fire wp_head, so without this direct
        // call all of that CSS silently never loads and the toolbar/modal
        // render as unstyled raw HTML.
        sqs_app_bar_styles( true );
    }
    ...
    <?php if ( $embedded ) : ?>
    /* Start Performance's own content container already adds 32px of
       padding around every view it renders (the platform-standard inset,
       matching every other page in that system) -- this plugin's own 22px
       on top of that is what was pushing the sticky toolbar down further
       than intended. Zero it out here since it's ours to remove safely,
       rather than fighting the platform's own padding (tried via a
       negative margin, reverted -- see the .sqs-quote-toolbar rule above). */
    .sqs-calc { padding-top: 0; }
    <?php endif; ?>
    .sqs-calc *,
    .sqs-calc *::before,
    ...
    <?php if ( $embedded ) : ?>
    /* Embedded contexts (e.g. Start Performance) have less horizontal room
       per KPI card than the standalone full-width page this size was
       designed for -- the standalone sizes overflow/clip here, so shrink
       them to fit the same 5-across row comfortably. */
    .sqs-calc .sqc-live-card .sqc-kpi .sqc-value { font-size:20px; }
    .sqs-calc .sqc-live-card .sqc-kpi:first-child .sqc-value { font-size:22px; }
    <?php endif; ?>
    ...
    .sqs-calc tr.sqc-override-row td { background:var(--sqc-warn-soft); color:var(--sqc-warn); font-weight:700; }
    .sqs-calc tr.sqc-override-row:hover td { background:var(--sqc-warn-soft); }
    ...
    <div class="sqs-calc" id="<?php echo esc_attr( $uid ); ?>">
        ...
        <!-- Quote action toolbar (kept even when embedded in another shell --
             functional buttons, not chrome/branding) -->
        ...
        <div class="sqc-bottom-stack">
            ...
            <div class="sqc-card sqc-screen-only" style="border-top:3px solid #0284c7;">
                ...
                <div class="sqc-qd-grid">
                    <div class="sqc-field"><label>Account / Company</label><input id="companyName" data-sqs-meta="companyName" placeholder="e.g. Acme Corp" autocomplete="off" /></div>
                    <div class="sqc-field"><label>Contact Name</label><input id="customerName" data-sqs-meta="customerName" placeholder="e.g. John Smith" autocomplete="off" /></div>
                    <div class="sqc-field"><label>Quote Date</label><input id="quoteDate" type="date" data-sqs-meta="quoteDate" /></div>
                    <div class="sqc-field"><label>Prepared By</label><input id="preparedBy" data-sqs-meta="preparedBy" placeholder="Rep name" autocomplete="off" /></div>
                    <div class="sqc-field"><label>Internal Reference</label><input id="internalRef" data-sqs-meta="internalRef" placeholder="Internal ref #" autocomplete="off" /></div>
    ...
    // A new/blank quote opens with no dimensions so the calculator reads $0
    // until the rep enters specs. Loading a saved quote restores real values.
    function freshForms() {
        const f = JSON.parse(JSON.stringify(DATA.defaults));
        ['tubing','inline','zipper'].forEach(function(p){
            if(!f[p]) return;
            f[p].width = 0;
            if('lengthFt' in f[p]) f[p].lengthFt = 0;
            if('lengthIn' in f[p]) f[p].lengthIn = 0;
            f[p].qty = 0;
            // Formula starts unselected (rather than pre-filled) so picking one
            // always fires a real change event -- syncResinCostFromFormula() only
            // runs on change, so a pre-selected formula left untouched silently
            // kept its own stale resinCost default instead of the formula's
            // actual cost. Resin Cost stays blank until a formula fills it in.
            f[p].formula = '';
            f[p].resinCost = 0;
        });
        return f;
    }
    function isEmptyQuote() {
        const f = state.forms[state.product] || {};
        const len = Number(f.lengthFt != null ? f.lengthFt : f.lengthIn) || 0;
        return !(((Number(f.width)||0) > 0) && len > 0 && ((Number(f.qty)||0) > 0) && !!f.formula);
    }
    function blankResult(r) {
        return Object.assign(r, {salesAmount:0,totalCost:0,profit:0,margin:0,unitPrice:0,pricePerLbs:0,weightPerUnit:0,orderWeight:0,setupCharge:0,materialCost:0,laborCost:0,packagingCost:0,specialtyCharge:0,overhead:0,targetSales:0,targetProfit:0,targetMargin:0,targetPricePerLbs:0,targetUnitPrice:0,priceBreaks:[]});
    }
    ...
    function setPreparedByDefault() {
        // Blank-check only -- never overwrites a value the rep already
        // typed or a value restored from a loaded quote. Convenience only,
        // not a tamper-proof audit trail (preparedBy is free text).
        const el = document.getElementById('preparedBy');
        if (el && !el.value && DATA.currentUserName) { el.value = DATA.currentUserName; }
    }
    ...
    // Formula gets its own select builder with a blank placeholder: picking a
    // formula must always be a real 'change' event so Resin Cost/LBS auto-fills
    // from it (see freshForms() note) instead of silently keeping a stale value.
    function makeFormulaSelect(id,label,options,value) {
        const opts=options.map(o=>`<option value="${String(o)}"${String(o)===String(value)?' selected':''}>${o}</option>`).join('');
        return `<div class="sqc-field"><label>${label}</label><select id="${id}"><option value=""${value?'':' selected'}>&mdash; Select Formula &mdash;</option>${opts}</select></div>`;
    }
    function makeNumber(id,label,value,step='any') {
        // autocomplete="off" -- without it, pressing the Up/Down arrow keys to
        // adjust the value instead opens Chrome's autofill-suggestion dropdown
        // for any field with saved form history (same class of bug fixed for
        // the text fields in 8.6.8).
        return `<div class="sqc-field"><label>${label}</label><input type="number" step="${step}" id="${id}" value="${value}" autocomplete="off" /></div>`;
    }
    ...
    function renderForms() {
        ...
        document.getElementById('inputs-tubing').innerHTML=`<div class="sqc-field-grid sqc-three">
            ...
            ${makeFormulaSelect('tubing-formula','Formula',DATA.formulaOptions,t.formula)}
            ...
            ${makeNumber('tubing-customSetupCharge','Custom Setup Charge',t.customSetupCharge||0,'0.01')}
            ...
            ${makeNumber('tubing-overheadPct','COGS Overhead %',t.overheadPct||0,'0.001')}
        ...
        document.getElementById('inputs-inline').innerHTML=`<div class="sqc-field-grid sqc-three">
            ...
            ${makeFormulaSelect('inline-formula','Formula',DATA.formulaOptions,i.formula)}
            ...
            ${makeNumber('inline-customSetupCharge','Custom Setup Charge',i.customSetupCharge||0,'0.01')}
            ...
            ${makeNumber('inline-overheadPct','COGS Overhead %',i.overheadPct||0,'0.001')}
        ...
        document.getElementById('inputs-zipper').innerHTML=`<div class="sqc-field-grid sqc-three">
            ...
            ${makeFormulaSelect('zipper-formula','Formula',DATA.formulaOptions,z.formula)}
            ...
            ${makeNumber('zipper-overheadPct','COGS Overhead %',z.overheadPct||0,'0.001')}
    ...
    function calculateTubing(f) {
        ...
        const pricePerLbs=orderWeight>0?salesAmount/orderWeight:0;
    ...
    function rowsToTable(rows) {
        return`<thead><tr><th>Item</th><th>Value</th></tr></thead><tbody>${rows.map(r=>`<tr${r[2]?` class="${r[2]}"`:''}><td>${r[0]}</td><td>${r[1]}</td></tr>`).join('')}</tbody>`;
    }
    ...
    function renderResults() {
        const r=getResults();
        const _empty=isEmptyQuote();
        if(_empty) blankResult(r);
        renderTopPricingCards();
        // productLabel lives inside the standalone app-bar, which is not
        // rendered when embedded (e.g. inside Start Performance) -- guard
        // so its absence there doesn't throw and abort the rest of this
        // function before the KPI values below get set.
        const productLabelEl=document.getElementById('productLabel');
        if (productLabelEl) productLabelEl.textContent=r.productName;
        ...
        if(_empty){
            document.getElementById('marginOutputTable').innerHTML='<thead><tr><th>Item</th><th>Value</th></tr></thead><tbody><tr><td colspan="2" style="padding:16px 12px;text-align:center;color:#64748b;font-style:italic">Enter product specs to see pricing</td></tr></tbody>';
            document.getElementById('targetOutputTable').innerHTML='<thead><tr><th>Item</th><th>Value</th></tr></thead><tbody><tr><td colspan="2" style="padding:16px 12px;text-align:center;color:#94a3b8;font-style:italic">&mdash;</td></tr></tbody>';
            document.getElementById('priceBreakTable').innerHTML='';
        ...
        const setupOverrideActive=customSetupValue>0;
        const overrideRowClass=setupOverrideActive?'sqc-override-row':'';
        ...
        document.getElementById('marginOutputTable').innerHTML=rowsToTable([
            ...
            ['Order Weight',num(r.orderWeight,4)+' lbs'],
            ['Custom Setup Charge'+(setupOverrideActive?' &#9888; overriding':''),money(customSetupValue),overrideRowClass],
            ['Setup Charge'+(setupOverrideActive?' (from override, not computed)':''),money(r.setupCharge),overrideRowClass],
...
// ─── Start Performance Platform Integration (optional) ────────────────────────
// Everything below is inert unless this plugin is running on a WordPress site
// that also has the "Start Performance" core plugin active (an unrelated,
// separate multi-module platform some clients run). On the live Fruth Custom
// Packaging site, Start Performance is not installed, so
// sqs_pricing_calculator_sp_mode() is always false and none of this code
// path ever executes -- the standalone behavior above is completely
// unaffected. See docs/CHANGELOG.md for the full design rationale.

function sqs_pricing_calculator_sp_mode() {
    return function_exists( 'sp_register_addon' );
}

/**
 * "Prepared By" auto-fill source. Display convenience only (see the caller
 * for the full caveat) -- in Start Performance mode this is the logged-in
 * team member's name; otherwise it's the standalone session's name, exactly
 * as before.
 */
function sqs_pricing_calculator_current_user_display_name() {
    if ( sqs_pricing_calculator_sp_mode() && function_exists( 'sp_get_current_team_member' ) ) {
        $member = sp_get_current_team_member();
        if ( $member && ! empty( $member->name ) ) {
            return sanitize_text_field( $member->name );
        }
    }
    return isset( $_SESSION['sqs_calc_current_user_name'] ) ? sanitize_text_field( $_SESSION['sqs_calc_current_user_name'] ) : '';
}

add_action( 'plugins_loaded', 'sqs_pricing_calculator_sp_boot', 25 );

function sqs_pricing_calculator_sp_boot() {
    if ( ! function_exists( 'sp_register_view' ) ) {
        return; // Start Performance core not present -- nothing to do.
    }
    sqs_pricing_calculator_sp_register();
}

function sqs_pricing_calculator_sp_register() {
    sp_register_addon( 'fruth-sales', array(
        'name'        => 'Fruth Quote Builder',
        'version'     => SQS_VERSION,
        'description' => 'Quote pricing for Tubing, In-Line BSB, and Zipper -- Fruth Custom Packaging.',
    ...
    sp_register_view( 'fruth-quotes',  SQS_PLUGIN_DIR . 'sp-views/fruth-quotes.php' );
    sp_register_view( 'fruth-pricing', SQS_PLUGIN_DIR . 'sp-views/fruth-pricing.php' );

    add_filter( 'sp_nav_items',     'sqs_pricing_calculator_sp_nav_items' );
    add_filter( 'sp_allowed_views', 'sqs_pricing_calculator_sp_allowed_views' );

    // Surface Fruth quote data in the SP dashboard, KPI board, and AI summary,
    // mirroring how SMTI's custom module does it (but reading the LOCAL sqs_quotes
    // table — Fruth's source of truth — instead of an external API).
    add_action( 'sp_dashboard_before_stats', 'sqs_pricing_calculator_sp_dashboard_stats' );
    add_filter( 'sp_intel_summary_lines',    'sqs_pricing_calculator_sp_intel_summary_lines', 20, 4 );
    add_filter( 'sp_intel_kpi_cards',        'sqs_pricing_calculator_sp_intel_kpi_cards' );
    add_action( 'sp_intel_after_kpi',        'sqs_pricing_calculator_sp_intel_product_panel' );
}

/**
 * Aggregates Fruth quotes using the LATEST revision per quote_number (each saved
 * revision is its own row sharing a quote_number). Returns overall totals plus
 * per-status and per-product breakdowns. "Active" = any status except Archived.
 */
function sqs_pricing_calculator_sp_quote_stats() {
    static $cache = null;
    if ( $cache !== null ) return $cache;
    global $wpdb;
    $t = sqs_pricing_calculator_quotes_table_name();
    $rows = $wpdb->get_results(
        "SELECT q.status, q.product_type, q.total_sales
           FROM $t q
           INNER JOIN (
               SELECT quote_number, MAX(revision_number) AS mrev
               FROM $t GROUP BY quote_number
           ) latest ON q.quote_number = latest.quote_number AND q.revision_number = latest.mrev",
        ARRAY_A
    );
    $stats = array(
        'count' => 0, 'value' => 0.0, 'active_count' => 0, 'active_value' => 0.0,
        'by_status' => array(), 'by_product' => array(),
    );
    foreach ( (array) $rows as $r ) {
        $status  = $r['status'] !== '' ? $r['status'] : 'Draft';
        $product = $r['product_type'] !== '' ? $r['product_type'] : 'Unspecified';
        $val     = (float) $r['total_sales'];
        $stats['count']++;
        $stats['value'] += $val;
        if ( ! isset( $stats['by_status'][ $status ] ) )   $stats['by_status'][ $status ]   = array( 'count' => 0, 'value' => 0.0 );
        if ( ! isset( $stats['by_product'][ $product ] ) ) $stats['by_product'][ $product ] = array( 'count' => 0, 'value' => 0.0 );
        $stats['by_status'][ $status ]['count']++;
        $stats['by_status'][ $status ]['value'] += $val;
        $stats['by_product'][ $product ]['count']++;
        $stats['by_product'][ $product ]['value'] += $val;
        if ( $status !== 'Archived' ) { $stats['active_count']++; $stats['active_value'] += $val; }
    }
    uasort( $stats['by_product'], 'sqs_pricing_calculator_sp_by_value_desc' );
    $cache = $stats;
    return $cache;
}

function sqs_pricing_calculator_sp_by_value_desc( $a, $b ) {
    if ( $a['value'] == $b['value'] ) return 0;
    return ( $a['value'] < $b['value'] ) ? 1 : -1;
}

// Quotes (latest revision) still open — Draft or Final, not Archived — whose latest
// revision hasn't been touched in $age_days days. Candidates to finalize / follow up.
// updated_at is stored in site-local time (current_time('mysql')), so compare against
// a site-local cutoff.
function sqs_pricing_calculator_sp_aging_quotes( $age_days = 30 ) {
    static $cache = array();
    $ckey = (string) $age_days;
    if ( isset( $cache[ $ckey ] ) ) return $cache[ $ckey ];
    global $wpdb;
    $t   = sqs_pricing_calculator_quotes_table_name();
    $now = current_time( 'timestamp' );
    $cutoff = date( 'Y-m-d H:i:s', $now - (int) $age_days * DAY_IN_SECONDS );
    $rows = $wpdb->get_results( $wpdb->prepare(
        "SELECT q.total_sales, q.updated_at
           FROM $t q
           INNER JOIN ( SELECT quote_number, MAX(revision_number) AS mrev FROM $t GROUP BY quote_number ) latest
             ON q.quote_number = latest.quote_number AND q.revision_number = latest.mrev
          WHERE q.status <> 'Archived' AND q.updated_at < %s",
        $cutoff
    ), ARRAY_A );
    $out = array( 'count' => 0, 'value' => 0.0, 'oldest_days' => 0 );
    foreach ( (array) $rows as $r ) {
        $out['count']++;
        $out['value'] += (float) $r['total_sales'];
        $age = (int) floor( ( $now - strtotime( $r['updated_at'] ) ) / DAY_IN_SECONDS );
...
// Dashboard "Custom Metrics" widget (top of the dashboard, before core stats).
function sqs_pricing_calculator_sp_dashboard_stats() {
    if ( function_exists( 'sp_is_view_hidden' ) && sp_is_view_hidden( 'fruth-quotes' ) ) return;
    $s = sqs_pricing_calculator_sp_quote_stats();
    if ( (int) $s['count'] === 0 ) return;
    $aging = sqs_pricing_calculator_sp_aging_quotes( 30 );
    $ac    = (int) $aging['count'];
    ?>
    <?php if ( $ac > 0 ) : ?>
    <a href="<?php echo esc_url( home_url( '/sp-app/?view=fruth-quotes' ) ); ?>" style="display:flex;align-items:center;gap:10px;text-decoration:none;background:#fffbeb;border:1px solid #fde68a;border-left:4px solid #f59e0b;border-radius:10px;padding:12px 16px;margin-bottom:14px;">
        <svg viewBox="0 0 20 20" fill="#d97706" width="18" height="18" style="flex:0 0 auto;"><path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm1-12a1 1 0 10-2 0v4a1 1 0 00.293.707l2.828 2.829a1 1 0 101.415-1.415L11 9.586V6z" clip-rule="evenodd"/></svg>
        <span style="color:#92400e;font-weight:700;"><?php echo number_format( $ac ); ?> quote<?php echo $ac === 1 ? '' : 's'; ?> ($<?php echo number_format( $aging['value'], 0 ); ?>) with no update in 30+ days — follow up</span>
        <span style="margin-left:auto;color:#b45309;font-weight:600;font-size:13px;">Review &rarr;</span>
    </a>
    <?php endif; ?>
    <div class="sp-stats sp-stats-2">
        <div class="sp-stat-card" style="border-top-color:#0ea5e9">
            <div class="sp-stat-value"><?php echo number_format( $s['active_count'] ); ?></div>
            <div class="sp-stat-label">Fruth Quotes</div>
        </div>
        <div class="sp-stat-card" style="border-top-color:#10b981">
            <div class="sp-stat-value">$<?php echo number_format( $s['active_value'] ); ?></div>
            <div class="sp-stat-label">Quote Value</div>
        </div>
    </div>
    <?php
}
...
function sqs_pricing_calculator_sp_intel_kpi_cards( $cards ) {
    $s = sqs_pricing_calculator_sp_quote_stats();
    if ( (int) $s['count'] === 0 ) return $cards;
    $aging = sqs_pricing_calculator_sp_aging_quotes( 30 );
    $ac    = (int) $aging['count'];

    $qsub    = 'active (Draft + Final)';
    $qaccent = 'accent';
    if ( $ac > 0 ) { $qsub = $ac . ' need follow-up (30+ days)'; $qaccent = 'warn'; }
    $cards[] = array( 'label' => 'Fruth Quotes', 'value' => number_format( $s['active_count'] ), 'sub' => $qsub, 'accent' => $qaccent );
    $cards[] = array( 'label' => 'Quote Value',  'value' => '$' . number_format( $s['active_value'], 0 ), 'sub' => 'active quotes', 'accent' => 'green' );

    // Needs Follow-up — always shown when the module has quotes (0 = "all current").
    $cards[] = array(
        'label'  => 'Needs Follow-up',
        'value'  => number_format( $ac ),
        'sub'    => $ac > 0 ? 'no update in 30+ days' : 'all quotes current',
        'accent' => $ac > 0 ? 'warn' : 'green',
    );
    return $cards;
}

// AI Summary lines — Fruth quote totals, status mix, and product-type breakdown.
function sqs_pricing_calculator_sp_intel_summary_lines( $lines, $days, $since, $today ) {
    $s = sqs_pricing_calculator_sp_quote_stats();
    if ( (int) $s['count'] === 0 ) return $lines;
    global $wpdb;
    $t = sqs_pricing_calculator_quotes_table_name();
    $new = (int) $wpdb->get_var( $wpdb->prepare( "SELECT COUNT(DISTINCT quote_number) FROM $t WHERE created_at >= %s", $since ) );

    $status_parts = array();
    foreach ( $s['by_status'] as $st => $d ) { $status_parts[] = $d['count'] . ' ' . $st; }
    $line = "- Fruth quotes: {$s['active_count']} active (\$" . number_format( $s['active_value'], 0 ) . ' total value)';
    if ( $status_parts ) $line .= ', status mix: ' . implode( ', ', $status_parts );
    if ( $new > 0 )      $line .= ", {$new} new in the last {$days} days";
    $lines[] = $line;

    $pp = array(); $n = 0;
    foreach ( $s['by_product'] as $prod => $d ) {
        $pp[] = $prod . ' (' . $d['count'] . ' quote' . ( $d['count'] != 1 ? 's' : '' ) . ', $' . number_format( $d['value'], 0 ) . ')';
        if ( ++$n >= 5 ) break;
    }
    if ( $pp ) $lines[] = "- Fruth quotes by product type: " . implode( '; ', $pp );

    $aging = sqs_pricing_calculator_sp_aging_quotes( 30 );
    if ( $aging['count'] > 0 ) {
        $lines[] = "- Aging Fruth quotes still open (Draft/Final) with no update in 30+ days: {$aging['count']} quote" . ( $aging['count'] != 1 ? 's' : '' ) . " (\$" . number_format( $aging['value'], 0 ) . ") — candidates to finalize or follow up.";
    }
    return $lines;
}

// KPI Dashboard panel — quotes broken down by product type (Fruth's equivalent of
// SMTI's distributor scorecard).
function sqs_pricing_calculator_sp_intel_product_panel() {
    if ( function_exists( 'sp_is_view_hidden' ) && sp_is_view_hidden( 'fruth-quotes' ) ) return;
    $s = sqs_pricing_calculator_sp_quote_stats();
    if ( empty( $s['by_product'] ) ) return;
    ?>
    <div class="sp-intel-section" style="margin-bottom:24px;">
        <h3 style="font-size:1.15rem;font-weight:800;letter-spacing:.01em;color:#0f172a;text-transform:none;margin-bottom:16px;">Quotes by Product Type</h3>
        <?php foreach ( $s['by_product'] as $prod => $d ) : ?>
            <div class="sp-intel-stat-row">
                <span><?php echo esc_html( $prod ); ?></span>
                <span style="font-weight:600;white-space:nowrap;">
                    <?php echo (int) $d['count']; ?> <span style="color:var(--sp-muted,#6b7280);font-weight:400;">quote<?php echo (int) $d['count'] !== 1 ? 's' : ''; ?></span>
                    &nbsp;·&nbsp; $<?php echo number_format( $d['value'], 0 ); ?>
                </span>
            </div>
        <?php endforeach; ?>
        <?php $aging = sqs_pricing_calculator_sp_aging_quotes( 30 ); if ( (int) $aging['count'] > 0 ) : ?>
        <div style="margin-top:12px;padding-top:12px;border-top:1px solid #e2e8f0;font-size:.85rem;">
            <span style="color:#b45309;font-weight:700;"><?php echo (int) $aging['count']; ?> quote<?php echo (int) $aging['count'] !== 1 ? 's' : ''; ?> need follow-up</span>
            <span style="color:#64748b;">&nbsp;·&nbsp;no update in 30+ days<?php if ( (int) $aging['oldest_days'] > 0 ) : ?> (oldest <?php echo (int) $aging['oldest_days']; ?> days)<?php endif; ?>&nbsp;·&nbsp;$<?php echo number_format( $aging['value'], 0 ); ?></span>
        </div>
        <?php endif; ?>
    </div>
    <?php
...
/**
 * Inserts Fruth's nav item(s) directly after the 'sales-core' section
 * header, matching the exact pattern SMTI Sales and Sales Core themselves
 * use to insert into that same section. "Quotes" is visible to every team
 * member (admin or agent); "Pricing Data" only to admins -- matching the
 * agreed role split. This only controls nav *visibility*; the actual
 * authorization check lives in each view file (see sp-views/fruth-pricing.php).
 */
function sqs_pricing_calculator_sp_nav_items( $items ) {
    $result = array();
    foreach ( $items as $item ) {
        ...
        if ( ! empty( $item['section'] ) && ! empty( $item['section_id'] ) && 'sales-core' === $item['section_id'] ) {
            ...
            if ( function_exists( 'sp_is_admin_member' ) && sp_is_admin_member() ) {
                $result[] = array(
                    'view'   => 'fruth-pricing',
                    'label'  => 'Fruth Pricing Data',
                    'custom' => true,
                    'icon'   => '<path fill="currentColor" fill-rule="evenodd" d="M11 3a1 1 0 10-2 0v1.055A6.002 6.002 0 004.055 9H3a1 1 0 100 2h1.055A6.002 6.002 0 009 15.945V17a1 1 0 102 0v-1.055A6.002 6.002 0 0015.945 11H17a1 1 0 100-2h-1.055A6.002 6.002 0 0011 4.055V3zM10 13a3 3 0 100-6 3 3 0 000 6z" clip-rule="evenodd"/>',
                );
            }
        }
    }
...
function sqs_pricing_calculator_sp_allowed_views( $views ) {
    $views[] = 'fruth-quotes';
    $views[] = 'fruth-pricing';
    return $views;
}
