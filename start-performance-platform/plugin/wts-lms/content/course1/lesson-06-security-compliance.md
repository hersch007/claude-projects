...
**Version:** 1.0 | 2026
**Preceding Lesson:** Lesson 5 — Component Types (NWF, TWF, NCM, ATF)
**Following Lesson:** Course 2 — AMP for Government / Jurisdiction Reviewers
...
- Why Two-Factor Authentication is mandatory for all AMP logins — and how it works
...
AMP is not just a workflow tool — it is a compliance platform. Every feature in AMP, from user login to document upload to final approval, is built around three interconnected principles: **security**, **regulatory compliance**, and **permanent record-keeping**.
...
The Official Record is AMP's permanent, tamper-evident log of everything that occurs within a project. It is not a feature — it is a federal requirement.

Under federal telecommunications law, local governments must maintain a complete and accessible record of all communications and decisions made regarding wireless facility applications. AMP fulfills this requirement automatically. Every action taken in AMP — every submission, every review comment, every revision request, every approval, every document uploaded — is written to the Official Record the moment it occurs.

> **Visual:** [INSERT IMAGE: AMP project detail showing the Official Record / activity log tab with chronological entries visible]
> *Caption: The Official Record tab in a project shows every action, every communication, and every document in chronological order — from project creation through final approval.*
...
- **All submitted data** — every field entered in the PIF and all components
- **All uploaded documents** — every PDF, engineering report, site plan, and certification submitted
- **All communications** — every comment, revision request, and response exchanged between Agent, Analyst, and Jurisdiction
- **All decisions** — every approval, rejection, revision request, and status change, with timestamps and user attribution
- **All user actions** — every login, edit, save, and submission, attributed to the specific authenticated user who took the action
- **All fee and invoice records** — every invoice generated and every payment confirmed
...
Federal telecommunications law — specifically the Telecommunications Act of 1996 and subsequent FCC regulations — requires that local governments provide a written record supporting any decision on a wireless facility application. A Jurisdiction that cannot produce documented evidence of its review process has no legal basis to defend its decisions in court or before a federal agency.

AMP satisfies this requirement by building the Official Record automatically and continuously throughout every project. No special action is required to create it — the Official Record exists by default for every project from the moment it is created.
...
|---|---|
| **Jurisdiction** | Provides the documented basis for every decision — legal defense against carrier challenges and federal agency review |
| **Agent / Carrier** | Creates a verifiable record that applications were submitted on time and exactly as submitted |
| **WTS / Analyst** | Documents all advisory actions and communications — protects WTS staff from liability disputes |
...
Once data enters the Official Record, it cannot be deleted or overwritten. Revisions are handled through a formal revision cycle — the original submission remains on record, and the revised version is added as a new entry with its own timestamp and user attribution. Nothing is erased.

> **This is why accuracy matters from the beginning.** Every error in a submission becomes a permanent part of the record. Corrections are possible — but they are visible, traceable, and part of the record. There is no undo button in AMP.
...
Two-Factor Authentication is mandatory for all AMP logins — no exceptions, for every user type, every session, every time.
...
5. Session begins — tied to the authenticated user account
...
2FA also satisfies baseline security requirements under federal frameworks governing how government-facing platforms must handle user authentication. This is not a configurable option — it is a platform requirement.

---

### Role-Based Access Control

AMP enforces strict role-based access. What a user can see, do, and submit in AMP depends entirely on their assigned role. This is not just a convenience feature — it is a security control that is enforced at the system level.
...
| **Analyst** | All projects in assigned jurisdiction(s) | No | No — advisory only | No |
| **Client (Jurisdiction)** | All projects in their jurisdiction | No | **Yes — exclusive authority** | No |
| **AIT (Analyst in Training)** | Same as Analyst | No | No — requires qualified Analyst co-sign | No |
| **Admin** | System-level; configurable | No | No | Yes |

> **Security Principle:** No user can see data they are not authorized to access. Agents cannot view other Agents' applications — even if those applications involve the same tower or same carrier. Analysts and Clients see everything within their assigned jurisdiction and nothing outside it.
...
This separation of duties is both a security requirement and a regulatory design principle. Final approval authority in AMP is exclusively reserved for the Jurisdiction (Client role) — and the system enforces this technically, not just by policy. No workaround exists.
...
- **What changed** — the state of the record before and after the action
...
AMP requires that specific documents be certified — meaning reviewed, stamped, and signed by a Professional Engineer (PE) licensed in the state where the project is located. Uploading uncertified versions of these documents will not satisfy review requirements and will result in a revision request.
...
| **Jurisdiction** | Verifies that the submitted FCC Compliance Document confirms compliance — does NOT set or modify standards |
...
> **Key Point:** Local governments verify FCC compliance — they do not regulate it. Attempting to impose local RF standards or deny an application based on RF concerns that federal standards already address may expose the Jurisdiction to a preemption challenge under federal telecommunications law.

---

### Performance Bonds

A Performance Bond is a financial instrument — similar to insurance — that guarantees the carrier will fulfill its obligations under the permit. In the wireless facility context, this is typically structured as a **removal bond**: it ensures that if the wireless facility is abandoned or decommissioned, the carrier has the financial obligation to remove the structure.
...
All applicants must provide a current Certificate of Insurance naming specific parties as **additional named insureds**. A generic insurance certificate that shows the carrier has insurance is not sufficient — the certificate must specifically name:
...
> **Insurance Rule:** Insurance expiration dates must be actively maintained throughout the life of the project — not just at time of submission. An expired certificate can halt a final inspection or block permit issuance. Agents are responsible for proactively notifying the Jurisdiction when insurance is renewed and uploading the updated certificate in AMP.

---

### Federal Shot Clock Compliance

The Shot Clock is a compliance requirement under federal law — not just a performance metric. (Lesson 3 introduced the Shot Clock as a dashboard feature; this section addresses its legal compliance dimensions.)
...
> **For Jurisdiction Users:** The Shot Clock is a federal compliance requirement. Failure to act within the Shot Clock window is not a missed deadline — it is a federal law violation with serious legal consequences. AMP displays Shot Clock status on the dashboard for every active project. Checking Shot Clock values should be part of every dashboard review session.
...
1. The Jurisdiction or Analyst sends the project back (status changes to **Revise** or **Awaiting Revision** — red on the dashboard)
...
This process preserves the complete history of the application — what was originally submitted, what was found to be incorrect, what was corrected, and when each action occurred. This history is legally significant and cannot be bypassed.

> **Practical Implication:** If a PIF was submitted with the wrong project type, it cannot simply be edited. It must go through a formal revision cycle — consuming time, resetting the Shot Clock in some cases, and creating additional review burden. The cost of getting it right the first time is always less than the cost of a revision cycle.

### The SAVE Requirement

AMP does not auto-save. Data that is entered but not saved before navigating away is permanently lost. This is not a temporary limitation — it is an intentional design choice that maintains the integrity of the Official Record. AMP records what is explicitly saved by the user, not what is temporarily displayed in a form field.

> **SAVE — SAVE — SAVE.** After every entry. After every document upload. Before navigating to any other screen. This applies to every user, every form, every session. Data that is not saved cannot be recovered.
...
The single best practice in AMP — applicable to every user type, every form, every session — is to **click Save after every single action before moving on.**

---

## Key Takeaways

- The **Official Record** is a federal requirement — AMP automatically and permanently documents every action, decision, and document for every project
- The **Official Record cannot be deleted or overwritten** — corrections go through formal revision cycles, and both the original and corrected versions remain on file
- **Two-Factor Authentication is mandatory** for all AMP logins — every user must have a text-capable mobile number registered to their account
- **Role-based access control** ensures each user sees only the data they are authorized to access — and only the Jurisdiction (Client role) can approve applications
- **Certified documents** must be stamped and signed by a PE licensed in the state where the project is located — the engineering firm's reputation does not substitute for this legal requirement
- **Local governments verify FCC RF compliance — they do not regulate it** — imposing local RF standards beyond what federal law allows may expose the Jurisdiction to legal challenge
- **The Shot Clock is a federal compliance requirement** — allowing it to expire results in deemed approval, loss of review authority, and potential federal court action
- **AMP does not auto-save** — click Save after every entry and every upload without exception

---

## Checkpoint Question

**Scenario:** A Jurisdiction reviewer receives a project with a structural analysis in the NCM Section 4. The document was prepared by a nationally recognized engineering firm. However, the reviewer notices it bears no PE stamp and no signature — just the firm's company letterhead. The reviewer says: *"This is a credible firm with a strong national reputation. I'll accept this document as-is and move forward with the review."*

What is the correct response?

**A)** The reviewer is correct — the reputation and track record of the engineering firm is sufficient evidence of the document's reliability. A PE stamp is a formality, not a legal requirement.

**B)** The reviewer should call the engineering firm directly and ask them to verbally confirm the report's findings. Verbal confirmation is equivalent to PE certification for review purposes.

**C)** The document must be rejected and the project sent back for revision. A structural analysis must be stamped and signed by a PE licensed in the project's state — the firm's reputation does not satisfy this legal requirement. An unstamped document means no individual engineer has taken professional or legal responsibility for its contents.

**D)** The reviewer can conditionally approve the structural component and request the PE stamp be submitted before the permit is issued.

---

**✓ Correct Answer: C** — PE certification is a legal standard, not a preference. The PE stamp and signature are the mechanism by which a licensed individual engineer takes professional and legal responsibility for the document's conclusions. A structural analysis prepared under company letterhead, with no PE stamp or signature, has no individual engineering accountability behind it — regardless of how credible the firm is. The Jurisdiction cannot responsibly approve work on a tower structure without a properly certified structural analysis confirming the tower can support the proposed load. The project must be sent back for a correctly stamped and signed structural analysis before review can continue.

---

*End of Lesson 6 | AMP Fundamentals Course Complete | Next: Course 2 — AMP for Government / Jurisdiction Reviewers*
