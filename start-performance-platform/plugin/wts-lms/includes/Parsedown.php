...
class Parsedown {
    ...
    public function text(string $text): string {
        ...
        while ($i < $total) {
            ...
            // Table — header row must contain a pipe, and the separator must too (so a
            // plain "---" stays a horizontal rule instead of being read as a separator).
            if (str_contains($line, '|') && isset($lines[$i+1]) && preg_match('/^\s*\|?[\s:-]*\|[\s:|-]*$/', $lines[$i+1])) {
                $headers = $this->tableRow($line);
                $i += 2; // skip separator
                $rows = '';
                while ($i < $total && str_contains($lines[$i], '|')) {
                    $rows .= '<tr>' . implode('', $this->tableRow($lines[$i], false)) . "</tr>\n";
                    $i++;
                }
