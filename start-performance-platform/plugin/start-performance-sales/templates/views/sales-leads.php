...
<div style="display:flex;gap:8px;margin-bottom:20px">
    <a href="<?php echo esc_url( home_url( '/sp-app/?view=leads' ) ); ?>"
       class="sp-btn <?php echo $leads_tab === 'list' ? 'sp-btn-primary' : 'sp-btn-ghost'; ?>"
       onclick="localStorage.setItem('sp_leads_tab','list')">List</a>
    <a href="<?php echo esc_url( home_url( '/sp-app/?view=leads&leads_tab=pipeline' ) ); ?>"
       class="sp-btn <?php echo $leads_tab === 'pipeline' ? 'sp-btn-primary' : 'sp-btn-ghost'; ?>"
       onclick="localStorage.setItem('sp_leads_tab','pipeline')">Pipeline</a>
</div>
<?php if ( ! isset( $_GET['leads_tab'] ) && ! isset( $_GET['action'] ) ) : ?>
<script>
(function(){
    var pref = localStorage.getItem('sp_leads_tab');
    if (pref === 'pipeline') {
        window.location.replace(<?php echo json_encode( home_url( '/sp-app/?view=leads&leads_tab=pipeline' ) ); ?>);
    }
})();
</script>
<?php endif; ?>
