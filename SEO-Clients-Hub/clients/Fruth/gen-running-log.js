// Fruth On-Page SEO Audit Running Log generator
// Data lives in pages-data.js. Run: node gen-running-log.js
...
body.push(new Paragraph({
  children: [new TextRun({ text: `Last updated: ${data.lastUpdated}   •   Pages audited: ${data.pages.length} of ${data.totalPages}   •   Verified live: ${verifiedCount}   •   Needs attention: ${data.pages.length - verifiedCount - failedCount}`, font: FONT, size: 22, color: ACCENT, bold: true })],
  spacing: { after: 160 },
...
if (data.guidelines && data.guidelines.length) {
  body.push(h2('Methodology & Guidelines'));
  data.guidelines.forEach(g => body.push(bullet(g)));
}

// ---------- GLOBAL ELEMENTS ----------
if (data.globalElements && data.globalElements.length) {
  body.push(h2('Global / Sitewide Elements (apply once)'));
  body.push(para('These elements repeat on every page. Set their alt text once and apply sitewide — they are not repeated in the per-page tables below.'));
...
for (const p of data.pages) {
  ...
  if (p.editorUrl) body.push(labelLine('HubSpot editor: ', p.editorUrl, { color: '0563C1' }));
  const kwRun = [run('Audited: ', { bold: true }), run(p.audited || '')];
  if (p.primaryKw) kwRun.push(run('      '), run('Primary keyword: ', { bold: true }), run(p.primaryKw));
  body.push(para(kwRun));
  if (p.secondaryKw) body.push(labelLine('Secondary keywords: ', p.secondaryKw));
  if (p.title) {
    body.push(labelLine(`Page Title (${p.title.chars} chars)`, ''));
    body.push(para([run(p.title.text, { size: 22, color: PRIMARY, bold: true })]));
  }
  if (p.meta) {
    body.push(labelLine(`Meta Description (${p.meta.chars} chars)`, ''));
  ...
  if (p.schemaLabel) body.push(para([run(p.schemaLabel, { bold: true })]));
  if (p.schema) codeBlock(p.schema).forEach(c => body.push(c));
  body.push(new Paragraph({ children: [], spacing: { after: 160 } }));
}
...
Packer.toBuffer(doc).then(buf => {
  fs.writeFileSync('FCP-On-Page-SEO-Audit-Running-Log.docx', buf);
