...
// Minimal ZIP builder with forward-slash entry names
const src    = path.resolve(__dirname, 'echo64-chatbot');
const outPath = path.resolve(__dirname, 'echo64-chatbot-2.6.5.zip');
...
const files = walkSync(src, 'echo64-chatbot');

// Build ZIP manually (STORE or DEFLATE)
const parts = [];
...
for (const { full, rel } of files) {
  ...
  const lfh = Buffer.concat([
    ...
    uint16LE(0),                          // flags
    uint16LE(useDeflate ? 8 : 0),        // method
...
for (const e of centralDir) {
  cdParts.push(Buffer.concat([
    ...
    uint16LE(20), uint16LE(20),          // ver made, ver needed
    uint16LE(0),                          // flags
    uint16LE(e.method),
    uint16LE(0), uint16LE(0),            // mod time/date
    uint32LE(e.crc),
    ...
    uint16LE(0), uint16LE(0),            // disk start, int attrib
    uint32LE(0),                          // ext attrib
    uint32LE(e.offset),
    e.nameBuf,
  ]));
}
...
const eocd = Buffer.concat([
  ...
  uint16LE(0), uint16LE(0),              // disk number, disk with CD
...
// Quick verify — list first 8 entries
const AdmZip = null; // skip, just check file size
const stat = fs.statSync(outPath);
console.log(`Size: ${(stat.size/1024).toFixed(1)} KB`);
console.log('Entry names (from build):');
files.slice(0,8).forEach(f => console.log(' ', f.rel));
