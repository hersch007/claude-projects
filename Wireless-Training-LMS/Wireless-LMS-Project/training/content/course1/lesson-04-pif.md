...
**Version:** 1.0 | 2026
**Preceding Lesson:** Lesson 3 â€” Projects, Dashboard, and Status
**Following Lesson:** Lesson 5 â€” Component Types (NWF, TWF, NCM, ATF)
...
- What information the PIF collects â€” and why each section matters
...
- Why accuracy in the PIF is critical â€” and what goes wrong when it isn't

---

## Overview

Every project in AMP begins with the Project Information Form â€” the PIF. Before any review can happen, before any components are activated, before any fees are calculated, the PIF must be completed and approved.
...
> **Key Point:** Get the PIF right, and the rest of the process flows efficiently. Get it wrong, and every step that follows is affected â€” wrong components, wrong fees, wrong reviewer, and avoidable delays.
...
- **Project Carrier Name** â€” the wireless carrier associated with the application
- **Applicant's Project Name** â€” the name the Agent assigns to this project
- **Applicant's Project Number** â€” the internal project number used by the Agent's organization
- **WTS Project Number** â€” a system-generated six-digit number assigned by AMP; this is the project's unique identifier in the WTS database and is consistent across all users who have access to the project
- **Project Description** â€” a brief description of what is being proposed

> **Note:** The WTS Project Number is assigned by the system â€” the Agent does not enter this. It becomes the official reference number for all communication and recordkeeping on this project.
...
| Eligible Facility / Colocation (EFC) | Colocation on an existing structure that meets federal law requirements â€” ensured approval if the structure remains safe |
| Eligible Facility / Modification (EFM) | Modification of equipment on an existing structure that meets federal requirements â€” ensured approval if the structure remains safe |
...
#### Eligible Facility Types â€” Special Guidance
...
- **Tower Owner Site ID or Unique Name** â€” a unique identifier used by the Agent to distinguish different tower sites. Note: these names are not regulated or reviewed by the municipality; the same site may be referenced by different names by different agents.
...
Once the PIF is submitted, it does not just create a record â€” it actively drives the entire application process. Here is what the PIF controls:
...
All PIF data becomes part of the permanent Official Record in AMP. Once submitted, this data cannot simply be changed â€” revisions require a formal revision cycle.

> **Visual:** [INSERT IMAGE: PIF workflow diagram showing PIF â†’ Invoice â†’ Components â†’ Review â†’ Approval]
> *Caption: The PIF is the gateway. Every downstream action â€” components, fees, review, approval â€” flows from what was entered here.*
...
6. **Agent begins filling out all required components immediately** â€” no waiting required *(Platform Update, May 2026)*
7. **Payment processing occurs** â€” may take several days depending on payment method
8. **Submission unlocks once payment is confirmed** â€” components cannot be submitted for review until payment clears
9. **Agent submits completed components** â€” Analyst and Jurisdiction review begins

> **Platform Update â€” "A Faster Path to Approval" (May 2026)**
> AMP was updated so agents can open and begin completing all required components as soon as the PIF is approved and the invoice is issued â€” without waiting for payment to clear. This eliminates processing delays during payment. However, no component can be submitted for Jurisdiction review until payment is confirmed.
...
## Why Accuracy Matters â€” Common PIF Errors
...
All system notifications â€” including invoice delivery, revision requests, and approval notices â€” are sent to the contacts entered in the PIF. Missing or incorrect contact information means critical communications go to the wrong person or nowhere at all.
...
- The PIF is the mandatory starting point for every project in AMP â€” no project begins without it
- The Project Type field is the most critical decision in the PIF â€” it controls components, fees, reviewer assignment, and Shot Clock calculation
- The WTS Project Number is system-generated and becomes the official identifier for the project across all users
- After PIF approval, agents can begin filling out all components immediately â€” but cannot submit until payment is confirmed
- Wrong information in the PIF creates cascading errors throughout the entire application process
- Always confirm the correct project type before submitting â€” contact your WTS Analyst if unsure
- All PIF data becomes part of the permanent Official Record and requires a formal revision cycle to change

---

*End of Lesson 4 | Next: Lesson 5 â€” Component Types (NWF, TWF, NCM, ATF)*
