<?php
/**
 * Quote Builder — Pricing Engine (ported into Start Performance).
 * Faithful port of standalone Quote Builder v8.5 (Start Advertising | RH Brashear).
 * All pricing math, rate tables, quote save/load and rendering are preserved.
 * Standalone scaffolding (shortcodes, wp-admin menus, PHP-session login, activation
 * hook) is neutralized; the SP module wrapper owns registration, auth and views.
 */
...
function sqs_pricing_calculator_user_can_quote_ajax() {
    // Start Performance integration: any authenticated SP team member (or admin) may quote.
...
// [SP] Activation handled by the module wrapper via the sp_activate hook.
...
// [SP] wp-admin menu removed; pricing tables are edited from the SP Quote Settings view.
...
function sqs_pricing_calculator_portal_url() {
    ...
    // [SP] Default "home" now returns to the Start Performance app.
    return home_url( '/sp-app/?view=dashboard' );
}
...
// [SP] app-bar styles are emitted by the SP view templates instead of wp_head.
...
// [SP] [sqs_portal] shortcode removed.
...
// [SP] wp-admin submenu removed; handled inside Start Performance.
...
// [SP] PHP-session bootstrap removed; Start Performance handles authentication.
...
// [SP] [sqs_pricing_data_admin] shortcode removed; editor rendered by the SP Quote Settings view.
...
// [SP] [sqs_pricing_calculator] shortcode removed; calculator rendered by the SP Quotes view.
