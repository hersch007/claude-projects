...
function sqsp_boot() {
	...
	// [SP] Print CSS/JS are emitted by the SP Quotes view (it calls sqsp_head_assets()),
	// not via wp_head, because the Start Performance app renders its own document shell.
}

// ── Admin settings (company address + quote terms) ────────────────────────────
// [SP] Print settings (company address / terms) are managed inside Start Performance.
...
function sqsp_head_assets() {
	...
	(function () {
		...
		function renderPrintQuote(r, state, quoteNumber) {
			...
			// Notes override the PHP-rendered default terms only if user typed something
