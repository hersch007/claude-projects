...
require_once __DIR__ . '/rallyop-safety.php';
require_once __DIR__ . '/rallyop-claim.php';
...
function rallyop_start_season() {
    ...
    $planned_ends_on = sanitize_text_field($_POST['planned_ends_on'] ?? '');
    $carryover = empty($_POST['carryover']) ? 0 : 1;
    ...
    if (!$active && $name && preg_match('/^\\d{4}-\\d{2}-\\d{2}$/', $starts_on)) {
        $wpdb->insert($table, ['group_id'=>$group_id,'name'=>$name,'starts_on'=>$starts_on,'planned_ends_on'=>$planned_ends_on ?: null,'carryover'=>$carryover,'created_by'=>get_current_user_id(),'created_at'=>current_time('mysql')], ['%d','%s','%s','%s','%d','%d','%s']);
    }
...
function rallyop_sessions_markup() {
    ...
    <section id="open-play-sessions" class="rop-sessions">
      ...
      <div class="rop-heading-row"><div><h2>Open Play Sessions</h2><p>Pick a time, rally the crew, and know whoÃ¢â‚¬â„¢s in.</p></div><button class="rop-primary" type="button" data-rop-toggle>Ã¯Â¼â€¹ SCHEDULE</button></div>
      <?php if (!empty($_GET['session_added'])): ?><div class="rop-notice">Session scheduled. YouÃ¢â‚¬â„¢re marked as in.</div><?php endif; ?>
      ...
      <?php foreach ($sessions as $session): $mine = $my_rsvps[(int)$session->id] ?? ''; $start = strtotime($session->starts_at); ?>
        <article class="rop-session-card">
          ...
          <div class="rop-session-info"><h3><?php echo esc_html($session->title); ?></h3><p><?php echo esc_html(wp_date('g:i A', $start)); ?><?php if ($session->location): ?> Ã‚Â· <?php echo esc_html($session->location); ?><?php endif; ?></p><?php if ($session->notes): ?><small><?php echo esc_html($session->notes); ?></small><?php endif; ?><div class="rop-count"><strong><?php echo absint($session->yes_count); ?> IN</strong><?php if ($session->capacity): ?> / <?php echo absint($session->capacity); ?> spots<?php endif; ?><?php if ($session->maybe_count): ?> Ã‚Â· <?php echo absint($session->maybe_count); ?> maybe<?php endif; ?></div></div>
          <form class="rop-rsvp" method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>">
            ...
            <?php foreach (['yes'=>'IÃ¢â‚¬â„¢M IN','maybe'=>'MAYBE','no'=>'CANÃ¢â‚¬â„¢T'] as $value=>$label): ?><button name="status" value="<?php echo esc_attr($value); ?>" class="<?php echo $mine === $value ? 'active' : ''; ?>"><?php echo esc_html($label); ?></button><?php endforeach; ?>
          </form>
        </article>
      <?php endforeach; ?>
      </div>
...
add_action('wp_enqueue_scripts', function() {
    ...
    wp_add_inline_script('rallyop-sessions', "(function(){function addComebacks(){var box=document.getElementById('rallyop-performance');if(!box)return;var marked=(box.dataset.comebacks||'').split(',').filter(Boolean),winner='Not marked yet';document.querySelectorAll('.rop-game-date').forEach(function(d){var row=d.closest('tr'),edit=row&&row.nextElementSibling,gameInput=edit&&edit.querySelector('input[name=game_id]'),cell=row&&row.lastElementChild;if(!gameInput||!cell||cell.querySelector('.rop-comeback-form'))return;var id=gameInput.value,isMarked=marked.indexOf(id)>=0,win=row.querySelector('.rop-game-team.winner strong');if(isMarked&&win)winner=win.textContent.trim();var form=document.createElement('form');form.className='rop-comeback-form';form.method='post';form.action=box.dataset.adminPost;[['action','rallyop_toggle_comeback'],['game_id',id],['group_id',box.dataset.groupId],['_wpnonce',box.dataset.comebackNonce]].forEach(function(v){var i=document.createElement('input');i.type='hidden';i.name=v[0];i.value=v[1];form.appendChild(i);});var b=document.createElement('button');b.type='submit';b.className='rop-comeback-btn'+(isMarked?' active':'');b.textContent=isMarked?'COMEBACK Ã¢Å“â€œ':'MARK COMEBACK';form.appendChild(b);cell.appendChild(form);});var cards=box.querySelectorAll('[data-rop-achievements]>div strong');if(cards.length===5)cards[3].textContent=winner;}setTimeout(addComebacks,500);setTimeout(addComebacks,1600);})();");
    $performance_encoding_fix = <<<'JS'
(function(){function fix(){var box=document.getElementById('rallyop-performance');if(!box)return;var games=[];document.querySelectorAll('.rop-game-date').forEach(function(d){var row=d.closest('tr'),teams=row&&row.querySelectorAll('.rop-game-team');if(!teams||teams.length!==2)return;function side(t){return{names:(t.querySelector('strong')?t.querySelector('strong').textContent:'').split('&').map(function(n){return n.trim();}).filter(Boolean),won:t.classList.contains('winner')};}var parsed=Date.parse(d.textContent.trim()+' '+new Date().getFullYear());games.push({a:side(teams[0]),b:side(teams[1]),time:Number.isFinite(parsed)?parsed:Date.now()});});var played={},weekly={},streaks={};games.forEach(function(g){[g.a,g.b].forEach(function(s){s.names.forEach(function(n){played[n]=(played[n]||0)+1;});});var win=g.a.won?g.a:g.b;win.names.forEach(function(n){if(g.time>=Date.now()-7*86400000)weekly[n]=(weekly[n]||0)+1;});});Object.keys(played).forEach(function(n){var run=0;for(var i=0;i<games.length;i++){var g=games[i],mine=g.a.names.indexOf(n)>=0?g.a:(g.b.names.indexOf(n)>=0?g.b:null);if(!mine)continue;if(!mine.won)break;run++;}streaks[n]=run;});function leader(map){return Object.keys(map).sort(function(a,b){return map[b]-map[a]||a.localeCompare(b);})[0]||'-';}var cards=box.querySelectorAll('[data-rop-achievements]>div strong');if(cards.length===5){cards[0].textContent=leader(weekly);var streak=leader(streaks);cards[1].textContent=streak==='-'?'-':streak+' - '+streaks[streak]+' straight';if(/[Ãâ�]/.test(cards[2].textContent))cards[2].textContent='No upset yet';cards[4].textContent=leader(played);}box.querySelectorAll('td').forEach(function(cell){if(!/[Ãâ�]/.test(cell.textContent))return;var record=cell.textContent.match(/^\s*(\d+)\D+(\d+)\s*$/);if(record)cell.textContent=record[1]+'-'+record[2];});box.querySelectorAll('th,strong,p,span').forEach(function(el){if(/[Ãâ�]/.test(el.textContent)&&el.closest('[data-rop-achievements]'))el.textContent='-';});}setTimeout(fix,1800);setTimeout(fix,3000);})();
JS;
    wp_add_inline_script('rallyop-sessions',$performance_encoding_fix);
    wp_add_inline_script('rallyop-sessions', "(function(){function cleanRecords(){document.querySelectorAll('#rallyop-performance td').forEach(function(cell){var text=cell.textContent.trim(),match=text.match(/^(\\d+).*?(\\d+)$/);if(match&&/[Ãâ]/.test(text))cell.textContent=match[1]+'-'+match[2];});}setTimeout(cleanRecords,2400);setTimeout(cleanRecords,4200);})();");
});
...
add_action('wp_footer', function() {
    ...
    (function(){
      ...
      function rate(w,l){return w+l?Math.round(w*100/(w+l))+'%':'Ã¢â‚¬â€';}
      function repair(){
        ...
        var pb=box.querySelector('[data-rop-player-stats]');if(pb){pb.textContent='';Object.keys(players).sort(function(a,b){return players[b].w-players[a].w;}).forEach(function(n){var p=players[n],r=document.createElement('tr');cell(r,n,'strong');cell(r,p.w+'Ã¢â‚¬â€œ'+p.l);cell(r,rate(p.w,p.l));cell(r,(p.pf-p.pa>0?'+':'')+(p.pf-p.pa));var recent=cell(r,'');p.form.slice(-5).forEach(function(x){var i=document.createElement('i');i.className='rop-form-dot '+x.toLowerCase();i.textContent=x;recent.appendChild(i);});pb.appendChild(r);});}
        var pairBody=box.querySelector('[data-rop-partners]');if(pairBody){pairBody.textContent='';Object.keys(pairs).sort(function(a,b){return pairs[b].w-pairs[a].w;}).forEach(function(n){var p=pairs[n],r=document.createElement('tr');cell(r,n,'strong');cell(r,p.w+'Ã¢â‚¬â€œ'+p.l);cell(r,rate(p.w,p.l));cell(r,(p.pf-p.pa>0?'+':'')+(p.pf-p.pa));pairBody.appendChild(r);});}
        var head=box.querySelector('[data-rop-headtohead]');if(head){head.textContent='';Object.keys(series).forEach(function(n){var s=series[n],r=document.createElement('tr');cell(r,n);cell(r,s.games);cell(r,s.w[0]+'Ã¢â‚¬â€œ'+s.w[1]);cell(r,(s.diff>0?'+':'')+s.diff);head.appendChild(r);});}
      }
...
// Keep engagement awards compact so they support the standings instead of dominating them.
add_action('wp_enqueue_scripts', function () {
    wp_add_inline_style('rallyop-sessions', '.rop-player-badge{padding:2px 5px!important;font-size:7px!important;letter-spacing:.03em!important}.rop-celebration-icon{font-size:32px!important}.rop-badges{gap:6px!important;margin:9px 0!important}.rop-badge{padding:7px 8px!important;border-radius:7px!important}.rop-badge span{font-size:8px!important}.rop-badge strong{margin-top:3px!important;font-size:12px!important;line-height:1.2!important}.rop-achievements{gap:7px!important;margin:16px 0!important}.rop-achievements>div{min-height:58px!important;padding:9px 10px!important;border-top-width:3px!important}.rop-achievements span{font-size:8px!important;letter-spacing:.07em!important}.rop-achievements strong{margin-top:6px!important;font-size:13px!important;line-height:1.2!important}.rop-comeback-btn{margin-left:4px!important;padding:3px 5px!important;font-size:7px!important}');
    wp_add_inline_style('rallyop-sessions', '.rop-player-badge img.emoji,.rop-player-badge svg,.rop-player-badge img{display:inline-block!important;width:9px!important;height:9px!important;min-width:9px!important;max-width:9px!important;margin:0 2px 0 0!important;object-fit:contain!important;vertical-align:-1px!important}.rop-rankings-feature tbody tr.rop-rank-leader td{background:#efffd0!important}.rop-rankings-feature tbody tr.rop-rank-close-second td{background:#f7ffe8!important;box-shadow:inset 0 1px #cbe787,inset 0 -1px #cbe787}.rop-rankings-feature tbody tr.rop-rank-close-second td:first-child{box-shadow:inset 4px 0 #caff4c,inset 0 1px #cbe787,inset 0 -1px #cbe787}');
    wp_add_inline_script('rallyop-sessions', "(function(){function closeRace(){var table=document.querySelector('.rop-rankings-feature table'),rows=table?Array.from(table.querySelectorAll('tbody tr')):[];rows.forEach(function(row){row.classList.remove('rop-rank-leader','rop-rank-close-second');});if(rows.length<2)return;var first=parseInt(rows[0].children[2]?rows[0].children[2].textContent:'',10),second=parseInt(rows[1].children[2]?rows[1].children[2].textContent:'',10);if(!Number.isFinite(first)||!Number.isFinite(second))return;rows[0].classList.add('rop-rank-leader');if(first-second<=10)rows[1].classList.add('rop-rank-close-second');}setTimeout(closeRace,700);setTimeout(closeRace,1800);})();");
    wp_add_inline_style('rallyop-sessions', '@media(max-width:760px){.rop-mobile-nav{grid-template-columns:repeat(4,1fr)!important}}');
    wp_add_inline_style('rallyop-sessions', '.rop-pop-board{margin:0 0 14px;padding:12px;border:1px solid #d9ddd3;border-radius:12px;background:#fff}.rop-pop-title{display:flex;justify-content:space-between;align-items:center;gap:8px;margin-bottom:9px}.rop-pop-title strong{font-size:16px}.rop-pop-title span{color:#d45131;font-size:9px;font-weight:950;letter-spacing:.12em}.rop-pop-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:7px}.rop-pop-card{min-width:0;padding:9px;border-radius:8px;background:#f4f7ef;border-left:3px solid #caff4c}.rop-pop-card b{display:block;margin-bottom:3px;color:#d45131;font-size:8px;letter-spacing:.08em}.rop-pop-card span{display:block;color:#173426;font-size:12px;font-weight:850;line-height:1.3}.rop-pop-card.rop-pop-wide{grid-column:1/-1;background:#173426}.rop-pop-card.rop-pop-wide b{color:#caff4c}.rop-pop-card.rop-pop-wide span{color:#fff}.rop-rank-shift{display:inline-block;margin-left:4px;padding:1px 4px;border-radius:8px;font-size:8px;font-weight:950}.rop-rank-shift.up{background:#dff6cb;color:#24601d}.rop-rank-shift.down{background:#f6ddd5;color:#8a301f}@media(max-width:480px){.rop-pop-grid{grid-template-columns:1fr}.rop-pop-card.rop-pop-wide{grid-column:auto}}');
    wp_add_inline_style('rallyop-sessions', '.rop-pop-card img.emoji,.rop-pop-card img{display:inline-block!important;width:13px!important;height:13px!important;min-width:13px!important;max-width:13px!important;margin:0 2px!important;object-fit:contain!important;vertical-align:-2px!important}.rop-medal{display:inline-grid;place-items:center;width:16px;height:16px;margin-right:2px;border-radius:50%;color:#173426;font-size:9px;font-style:normal;font-weight:950;vertical-align:1px}.rop-medal-1{background:#f4c84a}.rop-medal-2{background:#cbd2d8}.rop-medal-3{background:#d88a4b}');
    $pop_script = <<<'JS'
(function(){
  function playerName(cell){if(!cell)return'';var copy=cell.cloneNode(true);copy.querySelectorAll('.rop-player-badge,.rop-rank-shift,.rop-claim-pill').forEach(function(x){x.remove();});return copy.textContent.replace(/\u265b/g,'').trim();}
  function run(){
    var table=document.querySelector('.rop-rankings-feature table'),box=document.querySelector('.rop-rankings-feature');if(!table||!box||box.dataset.ropPopReady)return;
    var rows=Array.from(table.querySelectorAll('tbody tr')),players=rows.map(function(row,index){return{name:playerName(row.children[1]),rating:parseInt(row.children[2]?row.children[2].textContent:'1200',10)||1200,rank:index+1,row:row};}).filter(function(p){return p.name;});if(!players.length)return;
    box.dataset.ropPopReady='1';var group=new URLSearchParams(location.search).get('group')||'group',rankKey='rop-ranks-v1-'+group,bestKey='rop-bests-v1-'+group,oldRanks={},oldBests={};try{oldRanks=JSON.parse(localStorage.getItem(rankKey)||'{}');oldBests=JSON.parse(localStorage.getItem(bestKey)||'{}');}catch(e){}
    var newRanks={},newBests={},movers=[],personal=[];players.forEach(function(p){newRanks[p.name]=p.rank;newBests[p.name]=Math.max(oldBests[p.name]||0,p.rating);if(oldRanks[p.name]&&oldRanks[p.name]!==p.rank){var change=oldRanks[p.name]-p.rank,tag=document.createElement('span');tag.className='rop-rank-shift '+(change>0?'up':'down');tag.textContent=(change>0?'\u2191':'\u2193')+Math.abs(change);p.row.children[0].appendChild(tag);movers.push(p.name+' '+tag.textContent);}if(oldBests[p.name]&&p.rating>oldBests[p.name])personal.push(p.name+' reached a new high of '+p.rating);});
    try{localStorage.setItem(rankKey,JSON.stringify(newRanks));localStorage.setItem(bestKey,JSON.stringify(newBests));}catch(e){}
    var cards=[],gap=players.length>1?players[0].rating-players[1].rating:999;if(gap<=10)cards.push(['CLOSE RACE','Only '+gap+' point'+(gap===1?'':'s')+' separate '+players[0].name+' and '+players[1].name+'.','wide']);
    if(movers.length)cards.push(['RANK MOVEMENT',movers.slice(0,2).join(' &middot; '),'']);
    var ratingMap={};players.forEach(function(p){ratingMap[p.name]=p.rating;});var games=[];document.querySelectorAll('.rop-game-date').forEach(function(d){var row=d.closest('tr'),teams=row?row.querySelectorAll('.rop-game-team'):[];if(teams.length!==2)return;var sides=Array.from(teams).map(function(t){return{names:(t.querySelector('strong')?t.querySelector('strong').textContent:'').split('&').map(function(n){return n.trim();}).filter(Boolean),won:t.classList.contains('winner')};});var parsed=Date.parse(d.textContent.trim()+' '+new Date().getFullYear());games.push({sides:sides,time:Number.isFinite(parsed)?parsed:Date.now()});});
    if(games.length){var latest=games[0],win=latest.sides.filter(function(s){return s.won;})[0],lose=latest.sides.filter(function(s){return!s.won;})[0];if(win&&lose){var wr=win.names.reduce(function(s,n){return s+(ratingMap[n]||1200);},0)/Math.max(1,win.names.length),lr=lose.names.reduce(function(s,n){return s+(ratingMap[n]||1200);},0)/Math.max(1,lose.names.length);if(lr-wr>=15)cards.push(['UPSET ALERT',win.names.join(' & ')+' knocked off the higher-rated team!','']);}}
    var weekly={};games.filter(function(g){return g.time>=Date.now()-7*86400000;}).forEach(function(g){var win=g.sides.filter(function(s){return s.won;})[0];if(win)win.names.forEach(function(n){weekly[n]=(weekly[n]||0)+1;});});var podium=Object.keys(weekly).sort(function(a,b){return weekly[b]-weekly[a]||(ratingMap[b]||0)-(ratingMap[a]||0);}).slice(0,3);if(podium.length)cards.push(['WEEKLY PODIUM',podium.map(function(n,i){return'<i class="rop-medal rop-medal-'+(i+1)+'">'+(i+1)+'</i> '+n;}).join(' &middot; '),'']);
    if(personal.length)cards.push(['PERSONAL BEST',personal[0],'']);else{var best=players.slice().sort(function(a,b){return b.rating-a.rating;})[0];cards.push(['PERSONAL BEST',best.name+' leads at '+best.rating,'']);}
    var board=document.createElement('section');board.className='rop-pop-board';board.innerHTML='<div class="rop-pop-title"><strong>What&#8217;s popping</strong><span>LIVE CLUB MOMENTS</span></div><div class="rop-pop-grid">'+cards.map(function(c){return'<div class="rop-pop-card '+(c[2]?'rop-pop-'+c[2]:'')+'"><b>'+c[0]+'</b><span>'+c[1]+'</span></div>';}).join('')+'</div>';table.insertAdjacentElement('beforebegin',board);
  }
  setTimeout(run,2200);
})();
JS;
    wp_add_inline_script('rallyop-sessions', $pop_script);
    wp_add_inline_style('rallyop-sessions', '.rop-menu-install{display:inline-flex;align-items:center;justify-content:center;min-height:38px;padding:8px 11px;border:1px solid currentColor;border-radius:8px;background:transparent;color:inherit;font:inherit;font-size:11px;font-weight:900;cursor:pointer}.rop-mobile-install{width:100%;margin-top:8px;background:#caff4c!important;color:#173426!important;border:0!important}@media(max-width:760px){.site-nav>.rop-menu-install{display:none!important}}');
    wp_add_inline_script('rallyop-sessions', "(function(){function installed(){return window.matchMedia('(display-mode: standalone)').matches||window.navigator.standalone===true;}function openInstall(){var nativeButton=document.querySelector('[data-rop-install-button]'),help=document.querySelector('[data-rop-install-help]');if(nativeButton)nativeButton.click();if(help&&help.hidden)help.hidden=false;}function setup(){var nav=document.querySelector('.rop-mobile-nav');if(nav&&!nav.querySelector('[data-rop-highlights-nav]')){var bottom=document.createElement('button');bottom.type='button';bottom.setAttribute('data-rop-highlights-nav','1');bottom.dataset.icon='\u2605';bottom.textContent='Highlights';nav.appendChild(bottom);}if(!installed()){var desktop=document.querySelector('.site-nav');if(desktop&&!desktop.querySelector('.rop-menu-install')){var d=document.createElement('button');d.type='button';d.className='rop-menu-install';d.textContent='INSTALL RALLYOP';desktop.appendChild(d);}var mobile=document.getElementById('rop-mobile-menu');if(mobile&&!mobile.querySelector('.rop-menu-install')){var m=document.createElement('button');m.type='button';m.className='rop-menu-install rop-mobile-install';m.textContent='INSTALL RALLYOP';mobile.appendChild(m);}}}document.addEventListener('click',function(e){var highlight=e.target.closest('[data-rop-highlights-nav]');if(highlight){var launch=document.querySelector('.rop-highlights-launch');if(launch)launch.click();return;}if(e.target.closest('.rop-menu-install'))openInstall();});if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',setup);else setup();setTimeout(setup,950);})();");
}, 200);
