...
function rop_pwa_icon_url($size) {
    ...
    $bundled_icon = __DIR__ . '/rallyop-app-icon-' . $size . '.png';
    if (file_exists($bundled_icon)) {
        return add_query_arg('v', '6.4.1', plugin_dir_url(__FILE__) . basename($bundled_icon));
    }
...
add_action('template_redirect', function () {
    ...
    if ($path === 'rallyop-manifest.webmanifest' || isset($_GET['rop_manifest'])) {
        ...
        echo wp_json_encode([
            'id' => home_url('/'),
            'name' => 'RallyOP â€” Open Play With Friends',
            'short_name' => 'RallyOP',
...
add_action('wp_footer', function () {
    ...
    $group_id = max(0, absint($_GET['group'] ?? 0));
    ?>
    <div class="rop-install" data-rop-install hidden><button type="button" data-rop-install-button>ADD RALLYOP TO PHONE</button><button type="button" class="rop-install-close" aria-label="Dismiss install prompt">Ã—</button></div>
